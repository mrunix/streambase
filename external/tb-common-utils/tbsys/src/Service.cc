...
int Service::main(int argc, char* argv[]) {
  ...
  if (_cmd.compare("stop") == 0) {
    ...
    if (lfp >= 0) {
      ssize_t __attribute__((unused)) ret = read(lfp, buffer, 64);
      close(lfp);
      buffer[63] = '\0';
      p = strchr(buffer, '\n');
      if (p != NULL)
...
int Service::interruptCallback(int __attribute__((unused)) sig) {
  return EXIT_SUCCESS;
}
...
int Service::runDaemon(int argc , char* argv[]) {
  ...
  try {
    ...
    if (_closeFiles) {
      ...
      int __attribute__((unused)) fd = open("/dev/null", O_RDWR);
      assert(fd == 0);
      if (_chstdOut.empty()) {
        fd = dup2(0, 1);
        assert(fd == 1);
