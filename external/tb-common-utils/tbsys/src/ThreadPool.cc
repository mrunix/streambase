...
ThreadPool::~ThreadPool() {
  assert(_destroyed);
  _monitor.lock();
  TBSYS_LOG(DEBUG, "_workItem.size: %zu,_listSize: %d,_procSize: %d", _workItems.size(), _listSize, _procSize);
...
void ThreadPool::promoteFollower(pthread_t __attribute__((unused)) thid) {
  if (_sizeMax > 1) {
    this->lock();
    assert(!_promote);
    _promote = true;
