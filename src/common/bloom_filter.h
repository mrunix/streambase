 ...
 *
 */
#ifndef OCEANBASE_CHUNKSERVER_BLOOM_FILTER_H_
#define OCEANBASE_CHUNKSERVER_BLOOM_FILTER_H_
#include <limits.h>
#include <cmath>
#include <iostream>
#include <string.h>
 ...
 */
static const float BLOOM_FILTER_FALSE_POSITIVE_PROB = 0.1;
 ...
 public:
  ...
  void insert(const void* key, const int64_t len) {
    ...
    for (uint32_t i = 0; i < num_hash_functions_; ++i) {
      hash = hasher_(key, len, hash) % num_bits_;
      bloom_bits_[hash / CHAR_BIT] |= (1 << (hash % CHAR_BIT));
    }
  }
