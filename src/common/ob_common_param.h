...
#define SF_BIT_RESERVED             41
  ...
  union {
      ...
      struct {
        ...
        uint64_t direction_             :
        SF_BIT_DIRECTION;
        uint64_t not_exit_col_ret_nop_  :
        SF_BIT_NOT_EXIT_COL_RET_NOP;
        uint64_t daily_merge_scan_      :
        SF_BIT_DAILY_MERGE_SCAN;
        uint64_t full_row_scan_         :
        SF_BIT_FULL_ROW_SCAN;
        uint64_t rowkey_column_count_   :
        SF_BIT_ROWKEY_COLUMN_COUNT;
        uint64_t reserved_              :
        SF_BIT_RESERVED;
      };
    };
