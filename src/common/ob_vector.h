...
class ObVector {
 ...
 public:
  ObVector();
  explicit ObVector(int32_t size);
  virtual ~ObVector();
 public:
  int32_t size() const { return mem_end_ - mem_begin_; }
  int32_t capacity() const { return mem_end_of_storage_ - mem_begin_; }
  int32_t remain() const { return mem_end_of_storage_ - mem_end_; }
 public:
