...
DEFINE_SERIALIZE(ObiConfig) {
  ...
  } else if (OB_SUCCESS != (ret = serialization::encode_vi32(buf, buf_len, pos, static_cast<int32_t>(reserve1_)))) {
...
DEFINE_DESERIALIZE(ObiConfig) {
  ...
  } else if (OB_SUCCESS != (ret = serialization::decode_vi32(buf, data_len, pos, &reserve1_))) {
    TBSYS_LOG(ERROR, "deserialization error");
...
DEFINE_GET_SERIALIZE_SIZE(ObiConfig) {
  return serialization::encoded_length_vi32(read_percentage_)
         + serialization::encoded_length_vi32(reserve1_)
...
void ObiConfigEx::print() {
  char addr_buf[OB_IP_STR_BUFF];
  rs_addr_.to_string(addr_buf, OB_IP_STR_BUFF);
  TBSYS_LOG(INFO, "rs=%s read_percentage=%d", addr_buf, read_percentage_);
}

void ObiConfigEx::print(char* buf, const int64_t buf_len, int64_t& pos) const {
  char addr_buf[OB_IP_STR_BUFF];
  rs_addr_.to_string(addr_buf, OB_IP_STR_BUFF);
  databuff_printf(buf, buf_len, pos, "rs=%s read_percentage=%d|", addr_buf, read_percentage_);
}
