...
class CellInfoProcessor {
 public:
  inline CellInfoProcessor() : is_db_sem_(false), need_skip_(false), cur_op_type_(0) {
  };
  inline ~CellInfoProcessor() {
  };
 public:
  // 返回语法是否合法
  inline bool analyse_syntax(const common::ObMutatorCellInfo& mutator_cell_info) {
    bool bret = true;
