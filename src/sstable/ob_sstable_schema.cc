...
int ObSSTableSchema::update_hashmaps(const ObSSTableSchemaColumnDef& column_def) {
  ...
  if (OB_SUCCESS != (ret = update_table_schema_hashmap(column_def))) {
    TBSYS_LOG(WARN, "update table schema hashmap failed. table_id=%lu, column_group_id=%lu,"
              "column_name_id=%lu", column_def.table_id_,
              column_def.column_group_id_, column_def.column_name_id_);
  } else if (OB_SUCCESS != (ret = update_group_schema_hashmap(column_def))) {
    TBSYS_LOG(WARN, "update group schema hashmap failed. table_id=%lu, column_group_id=%lu,"
              "column_name_id=%lu", column_def.table_id_,
              column_def.column_group_id_, column_def.column_name_id_);
  } else if (OB_SUCCESS != (ret = update_group_index_hashmap(column_def))) {
    TBSYS_LOG(WARN, "update group index  hashmap failed. table_id=%lu, column_group_id=%lu,"
              "column_name_id=%lu", column_def.table_id_,
              column_def.column_group_id_, column_def.column_name_id_);
  }
  return ret;
  ...
  if (OB_SUCCESS == ret) {
    ...
    if (0 == size || (NULL != last && column_def.table_id_ != last->table_id_)) {
      ...
      if (-1 == ret_hash || hash::HASH_NOT_EXIST == ret_hash) {
        TBSYS_LOG(ERROR, "Get (table_id=%lu) from table schema hashmap failed", column_def.table_id_);
        ret = OB_ERROR;
        ...
        def->table_group_next_   = size;
        table_value.tail_offset_ = size;
        ++table_value.size_;
        ret_hash = table_schema_hashmap_.set(column_def.table_id_, table_value, 1); //overwrite
        if (-1 == ret_hash) {
          TBSYS_LOG(ERROR, "insert table_schema_hashmap_ failed table_id=%lu", column_def.table_id_);
          ret = OB_ERROR;
        }
      }
    }
  ...
  if (OB_SUCCESS == ret) {
    ...
    if (OB_SUCCESS == ret) {
      ...
      if (-1 == ret_hash) {
        ...
        index_value.head_offset_ = size;
        index_value.tail_offset_ = size;
        index_value.size_ = 1;
...
int ObSSTableSchema::add_column_def(const ObSSTableSchemaColumnDef& column_def) {
  ...
  if (OB_SUCCESS == ret) {
    ...
    if (OB_SUCCESS == ret) {
      column_def.offset_in_group_ = size - curr_group_head_;
    }
      ...
      if (hash_init_) {
        ...
        if (-1 == hash_get) {
          ...
          for (int32_t index = 0; index < group_set.size_; ++index) {
            ...
            if (offset < 0 || offset >= get_column_count()) {
              TBSYS_LOG(ERROR, "offset overflosw offset=%d total colum count=%ld",
                        offset, get_column_count());
              ret = OB_ERROR;
              break;
            } else {
    ...
    if (OB_SUCCESS == ret) {
      ...
      for (int64_t i = 0; OB_SUCCESS == ret && i < column_count; ++i) {
        ...
        if (OB_SUCCESS == ret) {
          ...
          if (OB_SUCCESS != ret) {
            TBSYS_LOG(WARN, "add column def failed cocolumn_def(table_id=%lu, column_group_id=%lu"
                      ", column_name_id=%lu", column_def.table_id_,
                      column_def.column_group_id_, column_def.column_name_id_);;
          }
        }
