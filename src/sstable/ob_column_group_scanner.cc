...
int ObColumnGroupScanner::next_cell() {
  ...
  if (OB_UNLIKELY(OB_SUCCESS != iret && OB_ITER_END != iret)) {
    TBSYS_LOG(ERROR, "next cell error iret=%d, group=%ld, table=%ld,"
              "sstable=%ld,cursor=%ld,current block count=%ld,iter status=%ld",
              iret, group_id_, scan_param_->get_table_id(),
...
int ObColumnGroupScanner::get_cell(ObCellInfo** cell) {
  return get_cell(cell, NULL);
}

int ObColumnGroupScanner::get_cell(ObCellInfo** cell, bool* is_row_changed) {
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(OB_SUCCESS != ret && NULL != sstable_reader_)) {
    TBSYS_LOG(ERROR, "get_cell error, ret=%d, group_id=%ld,sstable_id=%ld, iterate status=%d",
              ret, group_id_, sstable_reader_->get_sstable_id().sstable_file_id_, iterate_status_);
  }
...
int ObColumnGroupScanner::set_scan_param(
  ...
  if (NULL == scan_param || NULL == sstable_reader || !scan_param->is_valid()) {
    ...
    iret = OB_NOT_INIT;
                              ...
                              trans_input_column_id(group_id, group_seq, scan_param, sstable_reader))) {
...
int ObColumnGroupScanner::read_current_block_data(
  ...
  if (OB_SUCCESS == iret) {
    ...
    if (OB_SUCCESS != iret) {
      TBSYS_LOG(ERROR, "check record header error, iret=%d, block data size=%ld",
                iret, compressed_data_bufsiz);
    } else {
      // size not enough; realloc new memory.
      if (header.data_length_ > uncompressed_data_bufsiz_) {
        TBSYS_LOG(WARN, "block data length=%ld > fixed length=%ld, "
                  "block:(fileid=%ld,table_id=%ld,offset=%ld,size=%ld)",
