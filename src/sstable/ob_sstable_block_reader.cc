...
static int deserialize_sstable_rowkey(const char* buf, const int64_t data_len, ObString& key) {
  int ret = OB_SUCCESS;
  int64_t pos = 0;
  ...
  if ((NULL == buf) || (data_len <= 0)) {
    ...
    TBSYS_LOG(ERROR, "decode key length error, "
              "data_len=%ld, pos=%ld, key_length=%ld",
...
int ObSSTableBlockReader::deserialize(
  ...
  if ((NULL == data_buffer) || (data_bufsiz <= 0)) {
    ...
    if (index_entry_length > internal_bufsiz) {
      ...
      internal_buffer_ptr = GET_TSI(common::ModuleArena)->alloc_aligned(index_entry_length);
    }
...
bool ObSSTableBlockReader::Compare::operator()(
...
int ObSSTableBlockReader::get_row_key(const_iterator index, common::ObString& key) const {
...
int ObSSTableBlockReader::get_row(
  ...
  if (OB_SUCCESS != (ret = get_row_key(index, key))) {
    TBSYS_LOG(ERROR, "get row key error, ret=%d, format=%d, index=%ld,%ld",
...
int ObSSTableBlockReader::get_row_columns(const RowFormat format,
    ...
    while (row_start + pos < row_end) {
      ...
      if (OB_SUCCESS != ret) {
        TBSYS_LOG(ERROR, "deserialize id object error, ret=%d, "
                  "column_index=%ld, row_start=%ld, pos=%ld, size=%ld",
