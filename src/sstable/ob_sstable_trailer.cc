...
bool ObSSTableTrailer::is_valid() {
  ...
  if (size_ <= 0) {
    ...
    TBSYS_LOG(WARN, "table version error, table_version_=%d", table_version_);
  } else if (first_block_data_offset_ != 0) {
    TBSYS_LOG(WARN, "first block data offset error, first_block_data_offset_=%d",
    ...
    for (i = 0; i < RESERVED_LEN; ++i) {
      if (reserved64_[i] != 0) {
        TBSYS_LOG(WARN, "reserved64_ should be 0 [real:%ld,index:%d]", reserved64_[i], i);
        break;
      }
    }
    if (RESERVED_LEN == i) {
