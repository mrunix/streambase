...
int ObSSTableWriter::create_sstable(const ObSSTableSchema& schema,
  ...
  if (inited_) {
    ...
    TBSYS_LOG(WARN, "invalid arguments table_version= %ld table_attr=%p", table_version);
    ret = OB_ERROR;
...
int ObSSTableWriter::append_row(const ObSSTableRow& row,
  ...
  if (!inited_) {
    ...
    TBSYS_LOG(WARN, "invalid row, column_group_id", column_group_id);
    ret = OB_ERROR;
    ...
    TBSYS_LOG(WARN, "Dump key of last row in buffer and current row");
    hex_dump(cur_key_.ptr(), cur_key_.length(), true, TBSYS_LOG_LEVEL_WARN);
  ...
  if (OB_SUCCESS == ret) {
    ...
    if (OB_SUCCESS == ret
      ...
      cur_key_.assign(cur_key_buf_.get_buffer(), row_key_len);
    }
...
void ObSSTableWriter::set_file_sys(common::ObFileAppender& file_sys) {
  filesys_ = &file_sys;
}
...
int ObSSTableWriter::update_bloom_filter(const uint64_t column_group_id,
  ...
  if (OB_SUCCESS == (ret = bf_key_buf_.ensure_space(
    ...
    if (OB_SUCCESS == (ret = encode_i16(bf_key_buf, bf_key_size, pos, 0))
        && OB_SUCCESS == (ret = encode_i16(bf_key_buf, bf_key_size, pos, column_group_id))
        && OB_SUCCESS == (ret = encode_i32(bf_key_buf, bf_key_size, pos, table_id))) {
...
int ObSSTableWriter::write_record_header(const int16_t magic,
  ...
  if (NULL == comp_data || comp_size <= 0 || uncomp_size <= 0) {
    ...
    header.header_length_ = header_len;
     ...
     * it's wrong.
     */
    header.data_length_ = uncomp_size;
    header.data_zlength_ = comp_size;
  ...
  if (OB_SUCCESS == ret) {
    ...
    if (OB_SUCCESS != ret) {
      TBSYS_LOG(WARN, "Problem add entry to block index builder, "
                "table_id=%lu, record_size=%d, row_key: ",
                table_id_, record_size);
      hex_dump(cur_key_.ptr(), cur_key_.length(),
...
int ObSSTableWriter::write_schema() {
  int ret           = OB_SUCCESS;
  int64_t wrote_len = 0;
  int schema_size   = schema_.get_serialize_size();
  int64_t pos       = 0;
...
int ObSSTableWriter::write_trailer() {
  ...
  if (OB_SUCCESS == ret) {
    ...
    if (OB_SUCCESS != ret) {
      ...
      TBSYS_LOG(INFO, "close sstable='%s', checksum=%lu talbe_id=%lu",
                filename_, sstable_checksum_, table_id_);
      hex_dump(cur_key_.ptr(), cur_key_.length(),
               true, TBSYS_LOG_LEVEL_INFO);
    }
  }
...
int ObSSTableWriter::copy_schema(const ObSSTableSchema& schema) {
  int ret         = OB_SUCCESS;
  int column_num  = schema.get_column_count();
  ...
  for (int64_t i = 0; i < column_num; ++i) {
    ...
    if (NULL != column_def) {
      ...
      if (OB_SUCCESS != ret) {
        TBSYS_LOG(WARN, "add column def failed column_count=%d curr_ofset=%d",
                  column_num, i);
        break;
      }
    } else {
...
int ObSSTableWriter::store_trailer_fields(const uint64_t first_table_id,
  ...
  if (OB_SUCCESS == ret) {
    ret = trailer_.set_row_value_store_style(store_type);
  }
  ...
  if (is_dense_format()) {
    if (prev_column_group_row_count_ > 0
      ...
      TBSYS_LOG(WARN, "current column group row count isn't consistent "
                ...
                "pre_row_count=%d, cur_row_count=%ld, table_id=%lu, "
                "colum_group_id=%lu",
