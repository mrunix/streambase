...
class ObInsertStmt : public ObStmt {
 public:
  ObInsertStmt(sb::common::ObStringBuf* name_pool);
  ...
  int add_value_row(sb::common::ObArray<uint64_t>& value_row);
  bool is_replace() const;
  uint64_t get_table_id() const;
  uint64_t get_insert_query_id() const;
  int64_t get_value_row_size() const;
  const sb::common::ObArray<uint64_t>& get_value_row(int64_t idx) const;
 ...
 private:
  ...
  sb::common::ObArray<sb::common::ObArray<uint64_t> > value_vectors_;
};
...
inline int ObInsertStmt::add_value_row(sb::common::ObArray<uint64_t>& value_row) {
...
inline const sb::common::ObArray<uint64_t>& ObInsertStmt::get_value_row(int64_t idx) const {
