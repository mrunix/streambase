...
int ObMergerRpcProxy::init(ObMergerRpcStub* rpc_stub,
                           ObMergerSchemaManager* schema, ObMergerTabletLocationCache* cache, ObMergerServiceMonitor* monitor) {
  int ret = OB_SUCCESS;
      ...
      if (!range->border_flag_.inclusive_start() || range->border_flag_.is_min_value()) {
        ...
        *new_buffer = (char*)ob_malloc(length + 1, ObModIds::OB_MS_RPC);
        if (NULL == *new_buffer) {
          ...
          (*new_buffer)[length] = 0;
          search_key.assign(*new_buffer, length + 1);
        }
      }
    } else {
