 ...
 * Version: $Id$
 *
 * ob_merge_join_agent_imp.cc for ...
 *
 * Authors:
...
void sb::mergeserver::ObMergeJoinOperator::initialize() {
  ups_stream_ = NULL;
...
sb::mergeserver::ObMergeJoinOperator::ObMergeJoinOperator(ObMergerRpcProxy& rpc_proxy)
  : rpc_proxy_(&rpc_proxy) {
  initialize();
}

sb::mergeserver::ObMergeJoinOperator::~ObMergeJoinOperator() {
  clear();
}


bool sb::mergeserver::ObMergeJoinOperator::is_request_finished()const {
  return request_finished_;
}

void sb::mergeserver::ObMergeJoinOperator::clear() {
  //result_array_.clear();
...
void sb::mergeserver::ObMergeJoinOperator::prepare_next_round() {
  ObCellArray::clear();
}
...
char sb::mergeserver::ObMergeJoinOperator::ugly_fake_get_param_rowkey_ = 'a';

int sb::mergeserver::ObMergeJoinOperator::set_request_param(const ObScanParam& scan_param,
...
int sb::mergeserver::ObMergeJoinOperator::set_request_param(const ObGetParam& get_param,
                                                            ObUPSCellStream& ups_stream,
                                                            ObUPSCellStream& ups_join_stream,
                                                            const ObSchemaManagerV2& schema_mgr,
                                                            const int64_t max_memory_size) {
...
void sb::mergeserver::ObMergeJoinOperator::move_to_next_row(const ObGetParam& row_spec_arr,
...
int sb::mergeserver::ObMergeJoinOperator::prepare_join_param_() {
  int err = OB_SUCCESS;
...
int sb::mergeserver::ObMergeJoinOperator::apply_whole_row(const sb::common::ObCellInfo& cell, const int64_t row_beg) {
  int err = OB_SUCCESS;
...
int sb::mergeserver::ObMergeJoinOperator::merge_next_row(int64_t& cur_row_beg, int64_t& cur_row_end) {
  int err = OB_SUCCESS;
  ObCellInfo* cur_cell = NULL;
...
int sb::mergeserver::ObMergeJoinOperator::merge() {
  int err = OB_SUCCESS;
...
int sb::mergeserver::ObMergeJoinOperator::join_apply(const ObCellInfo& cell,
...
int sb::mergeserver::ObMergeJoinOperator::join() {
  int err = OB_SUCCESS;
  ObCellInfo* cur_cell = NULL;
  bool is_row_changed = false;
  ...
  sb::common::ObVector<int64_t>::iterator dst_width_it = join_row_width_vec_.begin();
...
int sb::mergeserver::ObMergeJoinOperator::next_cell() {
  int err = OB_SUCCESS;
  err = ObCellArray::next_cell();
...
int sb::mergeserver::ObMergeJoinOperator::get_next_rpc_result() {
  int err = OB_SUCCESS;
  if (NULL != get_param_) {
...
int sb::mergeserver::ObMergeJoinOperator::get_next_get_rpc_result() {
  int err = OB_SUCCESS;
  ...
  sb::common::ObIterator*        cur_cs_cell_it = NULL;
  /// get result from cs
  if (OB_SUCCESS == err) {
...
int sb::mergeserver::ObMergeJoinOperator::get_next_scan_rpc_result() {
  int err = OB_SUCCESS;
...
int sb::mergeserver::ObMergeJoinOperator::do_merge_join() {
  int err = OB_SUCCESS;
...
sb::mergeserver::ObGetMergeJoinAgentImp::~ObGetMergeJoinAgentImp() {
  clear();
}

int sb::mergeserver::ObGetMergeJoinAgentImp::set_request_param(const ObScanParam&,
    ObUPSCellStream&,
    ObUPSCellStream&,
...
int sb::mergeserver::ObGetMergeJoinAgentImp::set_request_param(const ObGetParam& get_param,
    ObUPSCellStream& ups_stream,
    ObUPSCellStream& ups_join_stream,
    const ObSchemaManagerV2& schema_mgr,
    const int64_t max_memory_size) {
...
int sb::mergeserver::ObGetMergeJoinAgentImp::get_cell(sb::common::ObCellInfo** cell) {
...
int sb::mergeserver::ObGetMergeJoinAgentImp::next_cell() {
  return merge_join_operator_.next_cell();
}

void sb::mergeserver::ObGetMergeJoinAgentImp::clear() {
  merge_join_operator_.clear();
}


bool sb::mergeserver::ObGetMergeJoinAgentImp::is_request_fullfilled() {
  return merge_join_operator_.is_request_finished();
}


bool sb::mergeserver::ObScanMergeJoinAgentImp::return_uncomplete_result_ = false;

sb::mergeserver::ObScanMergeJoinAgentImp::ObScanMergeJoinAgentImp(ObMergerRpcProxy& proxy)
  : merge_join_operator_(proxy) {
  pfinal_result_ = NULL;
  ups_stream_ = NULL;
}

sb::mergeserver::ObScanMergeJoinAgentImp::~ObScanMergeJoinAgentImp() {
  clear();
}

int sb::mergeserver::ObScanMergeJoinAgentImp::set_request_param(const ObScanParam& scan_param,
    ObUPSCellStream& ups_stream,
    ObUPSCellStream& ups_join_stream,
    const ObSchemaManagerV2& schema_mgr,
    const int64_t max_memory_size) {
...
int sb::mergeserver::ObScanMergeJoinAgentImp::set_request_param(const ObGetParam&,
    ObUPSCellStream&,
    ObUPSCellStream&,
    const ObSchemaManagerV2&,
    const int64_t) {
...
bool sb::mergeserver::ObScanMergeJoinAgentImp::is_request_fullfilled() {
  return merge_join_operator_.is_request_finished();
}

void sb::mergeserver::ObScanMergeJoinAgentImp::clear() {
  merge_join_operator_.clear();
...
int sb::mergeserver::ObScanMergeJoinAgentImp::filter_org_row_(const ObCellArray& cells, const int64_t row_beg,
...
int sb::mergeserver::ObScanMergeJoinAgentImp::jump_limit_offset_(ObIterator& cells,
...
int sb::mergeserver::ObScanMergeJoinAgentImp::prepare_final_result_() {
  int err  = OB_SUCCESS;
...
int sb::mergeserver::ObScanMergeJoinAgentImp::prepare_final_result_process_intermediate_result_() {
  int err = OB_SUCCESS;
...
int sb::mergeserver::ObScanMergeJoinAgentImp::next_cell() {
  int err = OB_SUCCESS;
  if (NULL == ups_stream_) {
...
int sb::mergeserver::ObScanMergeJoinAgentImp::get_cell(ObCellInfo** cell) {
