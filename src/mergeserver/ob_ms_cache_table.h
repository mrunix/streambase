    ...
    int operator - (const MapKey& key) const {
      ...
      if (common::OB_SUCCESS == ret) {
        ...
        if (range1.start_key_ == range1.end_key_) {
          ...
          TBSYS_LOG(DEBUG, "key[%.*s], begin[%.*s], end[%.*s], cmp[%d]", range1.start_key_.length(),
                    range1.start_key_.ptr(), range2.start_key_.length(), range2.start_key_.ptr(),
                    range2.end_key_.length(), range2.end_key_.ptr(), ret);
          ...
          TBSYS_LOG(DEBUG, "key[%.*s], begin[%.*s], end[%.*s], cmp[%d]", range2.start_key_.length(),
                    range2.start_key_.ptr(), range1.start_key_.length(), range1.start_key_.ptr(),
                    range1.end_key_.length(), range1.end_key_.ptr(), ret);
        } else {
