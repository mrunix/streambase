...
int ObMergerTabletLocationCache::init(const uint64_t mem_size, const uint64_t count,
  ...
  if (init_) {
    ...
    ret = tablet_cache_.init(mem_size, timeout, count);
    if (OB_SUCCESS != ret) {
  ...
  if (!init_) {
    ...
    if (NULL == temp) {
      ...
      *((uint64_t*) temp) = table_id;
      ...
      CacheKey.assign(temp, sizeof(table_id) + rowkey.length());
...
int ObMergerTabletLocationCache::set(const ObRange& range, const ObMergerTabletLocationList& location) {
  ...
  if (!init_) {
    ...
    ret = tablet_cache_.malloc(pair, size, location.get_serialize_size());
    if (OB_SUCCESS != ret) {
...
uint64_t ObMergerTabletLocationCache::size(void) {
