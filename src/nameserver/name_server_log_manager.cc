...
#include "name_server_log_manager.h"
...
int NameServerLogManager::init(NameServer* name_server, common::ObSlaveMgr* slave_mgr, const common::ObServer* server) {
  OB_ASSERT(name_server);
  OB_ASSERT(slave_mgr);
  OB_ASSERT(server);
  int ret = OB_SUCCESS;

  if (is_initialized_) {
    TBSYS_LOG(ERROR, "nameserver's log manager has been initialized");
    ret = OB_INIT_TWICE;
  }

  name_server_ = name_server;
  log_worker_.set_name_server(name_server_);
  log_worker_.set_balancer(name_server_->get_balancer());
  log_worker_.set_log_manager(this);

  const char* log_dir = name_server_->get_config().commit_log_dir;
  if (OB_SUCCESS == ret) {
    if (NULL == log_dir) {
  ...
  if (OB_SUCCESS == ret) {
    int64_t log_file_max_size = name_server_->get_config().max_commit_log_size;
    int64_t log_sync_type = name_server_->get_config().commit_log_sync_type;
    ...
    if (OB_SUCCESS != ret) {
      ...
      TBSYS_LOG(INFO, "NameServerLogMgr[this=%p] init succ[replay_point_=%lu max_log_id_=%lu]", this, replay_point_, max_log_id_);
    }
  }
...
int NameServerLogManager::replay_log() {
  ...
  if (ret == OB_SUCCESS && !is_log_dir_empty_) {
    if (ckpt_id_ > 0) {
      ...
      if (ret == OB_SUCCESS) {
        ret = name_server_->recover_from_check_point(rt_server_status_, ckpt_id_);
        if (ret != OB_SUCCESS) {
        ...
        while (ret == OB_SUCCESS) {
          if (OB_LOG_NOP != cmd) {
            ...
            if (ret != OB_SUCCESS) {
              ...
              //WARNING: master name server not exit when apply log failed
              // ::exit(OB_FATAL_EXIT);
            }
          }
          if (OB_SUCCESS == ret) {
...
int NameServerLogManager::do_check_point(const uint64_t checkpoint_id/* = 0 */) {
  ...
  if (ret == OB_SUCCESS) {
    ret = name_server_->do_check_point(ckpt_id);
  }
