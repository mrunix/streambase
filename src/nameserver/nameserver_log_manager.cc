...
#include "nameserver_log_manager.h"
...
int NameServerLogManager::init(NameServer* name_server, common::ObSlaveMgr* slave_mgr) {
  ...
  log_worker_.set_root_server(name_server_);
  log_worker_.set_log_manager(this);

  const char* log_dir = TBSYS_CONFIG.getString(STR_NAME_SECTION, OBRT_LOG_DIR_PATH);
  if (OB_SUCCESS == ret) {
    if (NULL == log_dir) {
      TBSYS_LOG(ERROR, "log directory is null, section: %s, name: %s", STR_NAME_SECTION, OBRT_LOG_DIR_PATH);
  ...
  if (OB_SUCCESS == ret) {
    int64_t log_file_max_size = TBSYS_CONFIG.getInt(STR_NAME_SECTION, OBRT_LOG_SIZE_MB, DEFAULT_LOG_SIZE_MB) * 1000 * 1000;
    int64_t log_sync_type = TBSYS_CONFIG.getInt(STR_NAME_SECTION, OBRT_LOG_SYNC_TYPE, DEFAULT_LOG_SYNC_TYPE);
...
int NameServerLogManager::replay_log() {
  ...
  if (OB_SUCCESS == ret) {
    ...
    if (OB_SUCCESS != ret) {
      ...
      TBSYS_LOG(INFO, "start log [%ld] done, cur_log_seq=%lu", max_log_id_, get_cur_log_seq());
    }
  }
  return ret;
}
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (err < 0 || err >= OB_MAX_FILE_NAME_LENGTH) {
      ...
      if (err < 0) {
        ...
        int st_str_len = 0;
        int server_status = name_server_->get_server_status();
