...
#include "nameserver/name_server_sql_proxy.h"
#include "nameserver/name_server_log_replay.h"
#include "nameserver/name_server_log_manager.h"
#include "nameserver/name_server_stat.h"
#include "nameserver/name_server_fetch_thread.h"
#include "nameserver/name_server_server_config.h"
#include "nameserver/name_server_inner_table_task.h"
...
class NameServerWorker : public common::ObBaseServer, public common::ObPacketQueueHandler {
 ...
 private:
  ...
  int ns_check_root_table(const int32_t version, common::ObDataBuffer& in_buff, easy_request_t* req, const uint32_t channel_id, common::ObDataBuffer& out_buff);
  ...
  int ns_dump_cs_tablet_info(const int32_t version, common::ObDataBuffer& in_buff, easy_request_t* req, const uint32_t channel_id, common::ObDataBuffer& out_buff);
  ...
  int rt_cs_delete_table_done(const int32_t version, common::ObDataBuffer& in_buff,
                              easy_request_t* conn, const uint32_t channel_id, common::ObDataBuffer& out_buff);
  int ns_cs_load_bypass_sstable_done(const int32_t version, common::ObDataBuffer& in_buff,
                                     easy_request_t* conn, const uint32_t channel_id, common::ObDataBuffer& out_buff);
...
inline ObConfigManager& NameServerWorker::get_config_mgr() {
  return config_mgr_;
}
