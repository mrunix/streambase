...
class NameServer;
class NameServerDDLOperator {
 public:
  NameServerDDLOperator();
  virtual ~NameServerDDLOperator();
 public:
  void init(NameServer* server, common::ObSchemaService* service);
  // create a new table
...
//
inline bool NameServerDDLOperator::check_inner_stat(void) const {
  return ((schema_client_ != NULL) && (name_server_ != NULL));
}
}
}
