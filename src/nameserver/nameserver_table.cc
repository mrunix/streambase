...
NameTable::NameTable(ObTabletInfoManager* tim): tablet_info_manager_(tim), sorted_count_(0) {
  meta_table_.init(ObTabletInfoManager::MAX_TABLET_COUNT, data_holder_);
}
NameTable::~NameTable() {
}

int NameTable::find_range(const common::ObRange& range,
...
int NameTable::find_key(const uint64_t table_id,
...
bool NameTable::table_is_exist(const uint64_t table_id) const {
  common::ObString key;
  const_iterator first;
  const_iterator last;
  const_iterator ptr;
  ...
  if (OB_SUCCESS == find_key(table_id, key, 1, first, last, ptr)) {
    const ObTabletInfo* tablet_info = get_tablet_info(ptr);
...
void NameTable::server_off_line(const int32_t server_index, const int64_t time_stamp) {
  int64_t count = 0;
...
void NameTable::dump_unusual_tablets(int64_t current_version, int32_t replicas_num) const {
  TBSYS_LOG(INFO, "meta_table_.size():%d", (int32_t)meta_table_.get_array_index());
...
const common::ObTabletInfo* NameTable::get_tablet_info(const const_iterator& it) const {
...
common::ObTabletInfo* NameTable::get_tablet_info(const const_iterator& it) {
  int32_t tablet_index = 0;
...
bool NameTable::move_back(const int32_t from_index_inclusive, const int32_t move_step) {
  bool ret = false;
  int32_t data_count = meta_table_.get_array_index();
  if ((move_step + data_count < ObTabletInfoManager::MAX_TABLET_COUNT) && from_index_inclusive <= data_count) {
    ...
    memmove(&(data_holder_[from_index_inclusive + move_step]), //dest
            ...
            (data_count - from_index_inclusive) * sizeof(NameMeta));  //size
...
int64_t NameTable::get_max_tablet_version(const const_iterator& it) {
...
int NameTable::modify(const const_iterator& it, const int32_t dest_server_index, const int64_t tablet_version) {
...
int NameTable::replace(const const_iterator& it,
...
NameTable::const_iterator NameTable::lower_bound(const common::ObRange& range) const {
...
NameTable::iterator NameTable::lower_bound(const common::ObRange& range) {
  iterator out = NULL;
  if (sorted_count_ > 0 && tablet_info_manager_ != NULL) {
    int32_t len = sorted_count_;
    int32_t half = 0;
...
int NameTable::add(const common::ObTabletInfo& tablet, const int32_t server_index, const int64_t tablet_version) {
  int ret = OB_ERROR;
  if (tablet_info_manager_ != NULL) {
    ret = OB_SUCCESS;
...
int NameTable::create_table(const common::ObTabletInfo& tablet, const int32_t* server_indexes, const int32_t replicas_num, const int64_t tablet_version) {
  int ret = OB_ERROR;
  ...
  if (tablet_info_manager_ != NULL) {
    ...
    if (OB_SUCCESS == ret && first == last) {
      insert_pos = first - begin();
    }
    if (OB_ERROR_OUT_OF_RANGE == ret && -1 == insert_pos) {
      ret = OB_SUCCESS;
      insert_pos = end() - begin();
    }
    if (OB_SUCCESS == ret) {
...
bool NameTable::check_lost_range() {
  bool ret = true;
  ObRange last_range;
  last_range.table_id_ = OB_INVALID_ID;
  last_range.border_flag_.set_max_value();
 ...
 */
int NameTable::shrink_to(NameTable* shrunk_table) {
  int ret = OB_ERROR;
  static char row_key_dump_buff[OB_MAX_ROW_KEY_LENGTH * 2];
  if (shrunk_table != NULL &&
    ...
    while (OB_SUCCESS == ret && it < end()) {
      ...
      if (OB_SUCCESS == ret) {
        ...
        last_tablet_index = shrunk_table->meta_table_.get_array_index();
      ...
      while (tablet_info->range_.equal(last_range)) {
        ...
        if (OB_SUCCESS != last_tablet_crc_helper->check_and_update(it->tablet_version_[0], tablet_info->crc_sum_)) {
          ...
          merge_one_tablet(shrunk_table, last_tablet_index, it);
        }
        it++;
        if (it >= end()) {
          break;
...
int32_t NameTable::find_suitable_pos(const const_iterator& it, const int32_t server_index, const int64_t tablet_version) {
  int32_t found_it_index = OB_INVALID_INDEX;
  for (int32_t i = 0; i < OB_SAFE_COPY_COUNT; i++) {
...
int NameTable::check_tablet_version(const int64_t tablet_version, int safe_copy_count) const {
  int ret = OB_SUCCESS;
  int tablet_ok = 0;
  const common::ObTabletInfo* tablet_info = NULL;
  for (int32_t i = 0; i < meta_table_.get_array_index(); i++) {
...
ObTabletCrcHistoryHelper* NameTable::get_crc_helper(const const_iterator& it) {
...
bool NameTable::check_tablet_copy_count(const int32_t copy_count) const {
  bool ret = true;
  int tablet_ok = 0;
  const common::ObTabletInfo* tablet_info = NULL;
  for (int32_t i = 0; i < meta_table_.get_array_index(); i++) {
...
void NameTable::merge_one_tablet(NameTable* shrunk_table, int32_t last_tablet_index, const_iterator it) {
  int32_t i = 0;
 ...
 */
int NameTable::get_range_pos_type(const common::ObRange& range, const const_iterator& first, const const_iterator& last) const {
...
int NameTable::split_range(const common::ObTabletInfo& reported_tablet_info, const const_iterator& pos,
  ...
  if (get_range_pos_type(range, pos, pos) == POS_TYPE_SPLIT_RANGE) {
    ...
    if (tablet_info == NULL || tablet_info_manager_ == NULL) {
      ...
      int32_t from_pos_inclusive = pos - begin();
...
int NameTable::add_range(const common::ObTabletInfo& reported_tablet_info, const const_iterator& pos,
                         const int64_t tablet_version, const int32_t server_index) {
  int ret = OB_ERROR;
  ObRange range = reported_tablet_info.range_;
  ObTabletInfo will_add_table;
...
int NameTable::write_to_file(const char* filename) {
  int ret = OB_SUCCESS;
  if (filename == NULL) {
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (ret == OB_SUCCESS) {
      ...
      header.header_length_ = header_length;
      header.version_ = 0;
      header.reserved_ = 0;

      header.data_length_ = total_size;
      header.data_zlength_ = total_size;
...
int NameTable::read_from_file(const char* filename) {
  int ret = OB_SUCCESS;
  if (filename == NULL) {
    ret = OB_INVALID_ARGUMENT;
...
void NameTable::dump_as_hex(FILE* stream) const {
  fprintf(stream, "root_table_size %ld sorted_count_ %d\n", meta_table_.get_array_index(), sorted_count_);
  NameTable::const_iterator it = this->begin();
  for (; it != this->end(); it++) {
    it->dump_as_hex(stream);
  }
}
void NameTable::read_from_hex(FILE* stream) {
  int64_t size = 0;
