...
#include "nameserver/name_server_admin2.h"
...
void usage() {
  printf("Usage: ns_admin -r <nameserver_ip> -p <nameserver_port> <command> -o <suboptions>\n");
  ...
  printf("\tset_obi_config -o ns_ip=<ns_ip>,ns_port=<ns_port>,read_percentage=<read_percentage>\n");
  ...
  printf("\nSee `ns_admin.log' for the detailed execution log.\n");
}
...
Command COMMANDS[] = {
  ...
  ,
  {
    "do_check_point", OB_RS_ADMIN_CHECKPOINT, do_ns_admin
  }
  ,
  {
    "reload_config", OB_RS_ADMIN_RELOAD_CONFIG, do_ns_admin
  }
  ,
  {
    "log_move_to_debug", OB_RS_ADMIN_INC_LOG_LEVEL, do_ns_admin
  }
  ,
  {
    "log_move_to_error", OB_RS_ADMIN_DEC_LOG_LEVEL, do_ns_admin
  }
  ,
  {
    "dump_root_table", OB_RS_ADMIN_DUMP_ROOT_TABLE, do_ns_admin
  },
  {
    "clean_root_table", OB_RS_ADMIN_CLEAN_ROOT_TABLE, do_ns_admin
  }
  ,
  {
    "dump_server_info", OB_RS_ADMIN_DUMP_SERVER_INFO, do_ns_admin
  }
  ,
  {
    "dump_migrate_info", OB_RS_ADMIN_DUMP_MIGRATE_INFO, do_ns_admin
  }
  ,
  {
    "dump_unusual_tablets", OB_RS_ADMIN_DUMP_UNUSUAL_TABLETS, do_ns_admin
  }
  ,
  {
    "check_schema", OB_RS_ADMIN_CHECK_SCHEMA, do_ns_admin
  }

  ,
  {
    "refresh_schema", OB_RS_ADMIN_REFRESH_SCHEMA, do_ns_admin
  }
  ,
  {
    "switch_schema", OB_RS_ADMIN_SWITCH_SCHEMA, do_ns_admin
  }
  ,
  {
    "clean_error_msg", OB_RS_ADMIN_CLEAN_ERROR_MSG, do_ns_admin
  }
  ,
  {
  ...
  ,
  {
    "enable_balance", OB_RS_ADMIN_ENABLE_BALANCE, do_ns_admin
  }
  ,
  {
    "disable_balance", OB_RS_ADMIN_DISABLE_BALANCE, do_ns_admin
  }
  ,
  {
    "enable_rereplication", OB_RS_ADMIN_ENABLE_REREPLICATION, do_ns_admin
  }
  ,
  {
    "disable_rereplication", OB_RS_ADMIN_DISABLE_REREPLICATION, do_ns_admin
  }
  ,
  {
  ...
  ,
  {
    "boot_strap", OB_RS_ADMIN_BOOT_STRAP, do_ns_admin
  }
  ,
  {
    "boot_recover", OB_RS_ADMIN_BOOT_RECOVER, do_ns_admin
  }
  ,
  {
    "init_cluster", OB_RS_ADMIN_INIT_CLUSTER, do_ns_admin
  }
  ,
  {
    ...
    while (-1 != (ch = getopt(argc, argv, "hVr:p:o:t:"))) {
      ...
      case 'o':
        ...
        while ('\0' != *subopts) {
          ...
          case OPT_RS_IP:
            if (NULL == value) {
              printf("option `ns_ip' needs an argument value\n");
              ret = OB_INVALID_ARGUMENT;
            } else {
              args.ups_ip = value;
            }
          ...
          case OPT_RS_PORT:
            if (NULL == value) {
              printf("option `ns_port' needs an argument value\n");
              ret = OB_INVALID_ARGUMENT;
            } else {
              args.ups_port = atoi(value);
            }
...
void Arguments::print() {
  TBSYS_LOG(INFO, "server_ip=%s port=%d", ns_host, ns_port);
...
int do_ns_admin(ObBaseClient& client, Arguments& args) {
  int ret = OB_SUCCESS;
  printf("do_ns_admin, cmd=%d...\n", args.command.pcode);
  static const int32_t MY_VERSION = 1;
  const size_t buff_size = sizeof(ObPacket) + 32;
  char buff[buff_size];
  ObDataBuffer msgbuf(buff, buff_size);
  // admin消息的内容为一个int32_t类型，定义在name_server_admin_cmd.h中
  if (OB_SUCCESS != (ret = serialization::encode_vi32(msgbuf.get_data(), msgbuf.get_capacity(), msgbuf.get_position(), args.command.pcode))) {
    ...
    if (OB_SUCCESS != (ret = result_code.deserialize(msgbuf.get_data(), msgbuf.get_capacity(), msgbuf.get_position()))) {
      ...
      printf("failed to do_ns_admin, err=%d\n", result_code.result_code_);
    } else {
      printf("Okay\n");
    }
  }
...
int do_ns_stat(ObBaseClient& client, Arguments& args) {
  int ret = OB_SUCCESS;
  printf("do_ns_stat, key=%d...\n", args.stat_key);
  static const int32_t MY_VERSION = 1;
  char* buff = new(std::nothrow) char[OB_MAX_PACKET_LENGTH];
  ObDataBuffer msgbuf(buff, OB_MAX_PACKET_LENGTH);
  // ns_stat消息的内容为一个int32_t类型，定义在name_server_stat_key.h中
  if (NULL == buff) {
    printf("no memory\n");
...
int do_cs_create_table(ObBaseClient& client, Arguments& args) {
  ...
  if ((1 > args.table_id) || (OB_INVALID_ID == args.table_id)) {
    ...
    usage();
    ret = OB_INVALID_ARGUMENT;
  } else if (!cs_addr.set_ipv4_addr(args.ns_host, args.ns_port)) {
    printf("invalid param, cs_host=%s cs_port=%d\n", args.ns_host, args.ns_port);
    usage();
    ret = OB_INVALID_ARGUMENT;
  } else {
    printf("force cs create empty table cs_ip=%s cs_port=%d version=%ld table_id=%lu...\n",
           args.ns_host, args.ns_port, args.table_version, args.table_id);
    static const int32_t MY_VERSION = 1;
