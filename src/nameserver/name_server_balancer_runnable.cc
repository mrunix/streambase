 ...
 * Version: $Id$
 *
 * name_server_balancer_runnable.cc
 *
 * Authors:
...
NameServerBalancerRunnable::~NameServerBalancerRunnable() {
}

void NameServerBalancerRunnable::wakeup() {
  balance_worker_sleep_cond_.broadcast();
}

bool NameServerBalancerRunnable::is_master() const {
  return role_mgr_.is_master();
}

void NameServerBalancerRunnable::run(tbsys::CThread* thread, void* arg) {
  UNUSED(thread);
  UNUSED(arg);
