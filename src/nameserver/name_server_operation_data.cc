...
#include "name_server_operation_data.h"
...
void NameServerOperationData::reset_data() {
  operation_type_ = common::INVALID_TYPE;
  delete_tables_.clear();
  root_table_.reset_root_table();
  schema_helper_.reset_schema_manager();
}
void NameServerOperationData::destroy_data() {
  operation_type_ = common::INVALID_TYPE;
  delete_tables_.clear();
  root_table_.destroy_data();
  schema_helper_.destroy_data();
}
OperationType NameServerOperationData::get_operation_type() {
...
int NameServerOperationData::generate_root_table() {
  int ret = OB_SUCCESS;
...
int NameServerOperationData::generate_bypass_data(const common::ObSchemaManagerV2* schema_mgr) {
  int ret = OB_SUCCESS;
  if (NULL == schema_mgr) {
...
common::ObArray<uint64_t>& NameServerOperationData::get_delete_table() {
  return delete_tables_;
}
int NameServerOperationData::init_schema_mgr(const common::ObSchemaManagerV2* schema_mgr) {
...
int NameServerOperationData::set_delete_table(common::ObArray<uint64_t>& delete_tables) {
  int ret = OB_SUCCESS;
...
int NameServerOperationData::check_root_table(common::ObTabletReportInfoList& delete_list) {
