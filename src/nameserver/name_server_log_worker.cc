...
#include "nameserver/name_server_log_worker.h"
#include "nameserver/name_server_server.h"
...
void NameServerLogWorker::set_name_server(NameServer* name_server) {
  name_server_ = name_server;
}

void NameServerLogWorker::set_balancer(NameServerBalancer* balancer) {
  balancer_ = balancer;
}
...
int NameServerLogWorker::sync_schema(const int64_t timestamp) {
  ...
  if (ret == OB_SUCCESS) {
    FileUtils fu;
    int32_t rc = fu.open(name_server_->config_.schema_filename, O_RDONLY);
    if (rc < 0) {
      ret = OB_ERROR;
...
int NameServerLogWorker::regist_cs(const ObServer& server, const char* server_version, const int64_t timestamp) {
  return log_server_with_ts(OB_RT_CS_REGIST, server, server_version, timestamp);
}
int NameServerLogWorker::regist_lms(const ObServer& server, int32_t sql_port, const char* server_version, const int64_t timestamp) {
  int ret = OB_SUCCESS;
...
int NameServerLogWorker::regist_ms(const ObServer& server, int32_t sql_port, const char* server_version, const int64_t timestamp) {
  int ret = OB_SUCCESS;
...
int NameServerLogWorker::log_server_with_ts(const LogCommand cmd, const ObServer& server, const char* server_version, const int64_t timestamp) {
  int ret = OB_SUCCESS;
...
int NameServerLogWorker::add_range_for_load_data(const common::ObTabletReportInfoList& tablets) {
  int ret = OB_SUCCESS;
...
int NameServerLogWorker::delete_replicas(const ObServer& server, const ObTabletReportInfoList& replicas) {
  int ret = OB_SUCCESS;
  char* log_data = NULL;
  log_data = static_cast<char*>(ob_malloc(OB_MAX_PACKET_LENGTH, ObModIds::OB_RS_LOG_WORKER));
  if (log_data == NULL) {
...
int NameServerLogWorker::remove_replica(const ObTabletReportInfo& replica) {
  int ret = OB_SUCCESS;
  char* log_data = NULL;
  log_data = static_cast<char*>(ob_malloc(OB_MAX_PACKET_LENGTH, ObModIds::OB_RS_LOG_WORKER));
  if (log_data == NULL) {
...
int NameServerLogWorker::remove_table(const common::ObArray<uint64_t>& deleted_tables) {
  int ret = OB_SUCCESS;
  char* log_data = NULL;
  log_data = static_cast<char*>(ob_malloc(OB_MAX_PACKET_LENGTH, ObModIds::OB_RS_LOG_WORKER));
  int64_t pos = 0;
...
int NameServerLogWorker::delete_load_table(const uint64_t table_id, const int32_t status, const int64_t end_time) {
  int ret = OB_SUCCESS;
  char* log_data = NULL;
  log_data = static_cast<char*>(ob_malloc(OB_MAX_PACKET_LENGTH, ObModIds::OB_RS_LOG_WORKER));
  int64_t pos = 0;
...
int NameServerLogWorker::add_new_tablet(const ObTabletInfo tablet, const ObArray<int32_t>& chunkservers, const int64_t mem_version) {
  int ret = OB_SUCCESS;
...
int NameServerLogWorker::init_first_meta(const ObTabletMetaTableRow& first_meta_row) {
  int ret = OB_SUCCESS;
...
int NameServerLogWorker::clean_root_table() {
  int ret = OB_SUCCESS;
...
int NameServerLogWorker::got_config_version(int64_t config_version) {
  int ret = OB_SUCCESS;
  int64_t pos = 0;
  char* log_data = static_cast<char*>(ob_malloc(OB_MAX_PACKET_LENGTH, ObModIds::OB_RS_LOG_WORKER));
...
int NameServerLogWorker::sync_us_frozen_version(const int64_t frozen_version, const int64_t last_frozen_time) {
  int ret = OB_SUCCESS;
...
int NameServerLogWorker::do_cs_regist(const char* log_data, const int64_t& log_length) {
  ...
  if (ret == OB_SUCCESS) {
    ...
    ret = name_server_->regist_chunk_server(server, server_version, status, csr_ts);
  }

  return ret;
}
int NameServerLogWorker::do_lms_regist(const char* log_data, const int64_t& log_length) {
  int ret = OB_SUCCESS;
  ...
  if (ret == OB_SUCCESS) {
    ret = name_server_->regist_merge_server(server, sql_port, true, server_version, msr_ts);
  }
...
int NameServerLogWorker::do_ms_regist(const char* log_data, const int64_t& log_length) {
  ...
  if (ret == OB_SUCCESS) {
    ...
    ret = name_server_->regist_merge_server(server, sql_port, false, server_version, msr_ts);
  }
...
int NameServerLogWorker::do_server_down(const char* log_data, const int64_t& log_length) {
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (it != NULL) {
      ...
      if (name_server_->root_table_ != NULL) {
        name_server_->root_table_->server_off_line(static_cast<int32_t>(it - name_server_->server_manager_.begin()), sd_ts);
        ret = OB_SUCCESS;
      }
    }
  }
...
int NameServerLogWorker::do_cs_migrate_done(const char* log_data, const int64_t& log_length) {
  ...
  if (ret == OB_SUCCESS) {
    ret = name_server_->migrate_over(result, desc,
...
int NameServerLogWorker::do_add_range_for_load_data(const char* log_data, const int64_t& log_length) {
  int ret = OB_SUCCESS;
  ...
  if (ret == OB_SUCCESS) {
    ret = name_server_->add_range_to_root_table(tablets, is_replay_log);
    if (OB_SUCCESS != ret) {
...
int NameServerLogWorker::do_add_new_tablet(const char* log_data, const int64_t& log_length) {
  ...
  if (ret == OB_SUCCESS) {
    if (OB_SUCCESS != (ret = name_server_->create_new_table(true, tablet, chunkservers, mem_version))) {
...
int NameServerLogWorker::do_batch_add_new_tablet(const char* log_data, const int64_t& log_length) {
  int ret = OB_SUCCESS;
  ...
  if (ret == OB_SUCCESS) {
    ret = name_server_->slave_batch_create_new_table(tablets, server_index,
...
int NameServerLogWorker::do_remove_replica(const char* log_data, const int64_t& log_length) {
  int ret = OB_SUCCESS;
  ...
  if (OB_SUCCESS != (ret = tablet.deserialize(log_data, log_length, pos))) {
    ...
    // replay log
    ret = name_server_->remove_replica(true, tablet);
    if (ret != OB_SUCCESS) {
...
int NameServerLogWorker::do_delete_replicas(const char* log_data, const int64_t& log_length) {
  int ret = OB_SUCCESS;
  ObServer server;
  ...
  if (ret != OB_SUCCESS) {
    ...
    } else if (OB_SUCCESS != (ret = name_server_->delete_replicas(true, server, replicas))) {
...
int NameServerLogWorker::do_remove_table(const char* log_data, const int64_t& log_length) {
  int ret = OB_SUCCESS;
  ...
  if (OB_SUCCESS != (ret = serialization::decode_vi64(log_data, log_length, pos, &table_count))) {
    ...
    if (OB_SUCCESS == ret) {
      ...
      ret = name_server_->delete_tables(true, list);
      if (ret != OB_SUCCESS) {
...
int NameServerLogWorker::do_add_load_table(const char* log_data, const int64_t& log_length) {
  OB_ASSERT(balancer_);
  int ret = OB_SUCCESS;
  ObString table_name;
  uint64_t table_id = 0;
...
int NameServerLogWorker::do_delete_load_table(const char* log_data, const int64_t& log_length) {
  OB_ASSERT(balancer_);
  int ret = OB_SUCCESS;
  uint64_t table_id = 0;
  int32_t status = 0;
...
int NameServerLogWorker::do_cs_merge_over(const char* log_data, const int64_t& log_length) {
  ...
  if (ret == OB_SUCCESS) {
    //ignore return value
    name_server_->waiting_job_done(server, cs_merge_ts);
  }
...
int NameServerLogWorker::do_sync_frozen_version(const char* log_data, const int64_t& log_length) {
  ...
  if (ret == OB_SUCCESS) {
    ret = name_server_->report_frozen_memtable(frozen_version, 0, true);
  }
  return ret;
}

int NameServerLogWorker::do_init_first_meta_row(const char* log_data, const int64_t& log_length) {
  int ret = OB_SUCCESS;
  ...
  if (ret == OB_SUCCESS) {
    if (name_server_->get_boot_state() == ObBootState::OB_BOOT_OK) {
      TBSYS_LOG(WARN, "name server is boot ok alreay, not need to replay init log.");
    } else if (OB_SUCCESS != (ret = name_server_->get_first_meta()->init(row))) {
      TBSYS_LOG(WARN, "fail to replay init first meta file. err=%d", ret);
    } else {
      TBSYS_LOG(INFO, "name server boot strap success. set boot ok");
      name_server_->get_boot()->set_boot_ok();
    }
  }
  return ret;
...
int NameServerLogWorker::do_sync_frozen_version_and_time(const char* log_data, const int64_t& log_length) {
  int ret = OB_SUCCESS;
  ...
  if (ret == OB_SUCCESS) {
    ret = name_server_->report_frozen_memtable(frozen_version, last_frozen_time, true);
  }
  return ret;
}
...
int NameServerLogWorker::do_clean_root_table(const char* log_data, const int64_t& log_length) {
  int ret = OB_SUCCESS;
  int64_t pos = 0;
  int64_t unused_value = 0;
  if (ret == OB_SUCCESS) {
  ...
  ret = name_server_->slave_clean_root_table();
  if (OB_SUCCESS != ret) {
    TBSYS_LOG(WARN, "fail to clean root table for slave_nameserver, ret=%d", ret);
  }
  return ret;
}

int NameServerLogWorker::do_got_config_version(const char* log_data, const int64_t& log_length) {
  int ret = OB_SUCCESS;
  ...
  if (NULL == name_server_) {
    TBSYS_LOG(ERROR, "name server is NULL!");
  } else if (NULL == (config_mgr = name_server_->get_config_mgr())) {
    TBSYS_LOG(ERROR, "Config manager is NULL!");
  } else if (NULL == name_server_->worker_) {
    TBSYS_LOG(ERROR, "root worker is NULL!");
  } else {
    ObRoleMgr::Role role = name_server_->worker_->get_role_manager()->get_role();
...
int NameServerLogWorker::set_frozen_version_for_brodcast(const int64_t bypass_version) {
  int ret = OB_SUCCESS;
  int64_t pos = 0;
  char* log_data = static_cast<char*>(ob_malloc(OB_MAX_PACKET_LENGTH, ObModIds::OB_RS_LOG_WORKER));
...
int NameServerLogWorker::do_set_bypass_version(const char* log_data, const int64_t& log_length) {
  int ret = OB_SUCCESS;
  int64_t pos = 0;
  ...
  if (OB_SUCCESS != (ret = serialization::decode_vi64(log_data, log_length, pos, &bypass_version))) {
    ...
    name_server_->set_bypass_version(bypass_version);
  }
  return ret;
}
