...
#include "nameserver/name_server_callback.h"
#include "nameserver/name_server_worker.h"
#include "nameserver/name_server_admin_cmd.h"
#include "nameserver/name_server_util.h"
#include "nameserver/name_server_stat_key.h"
#include "nameserver/name_server_bootstrap.h"
...
int NameServerWorker::initialize() {
  ...
  ObServer vip_addr;
  uint32_t vip = tbsys::CNetUtil::getAddr(config_.name_server_ip);
  int32_t local_ip = tbsys::CNetUtil::getLocalAddr(dev_name_);
  if (ret == OB_SUCCESS) {
    if (!vip_addr.set_ipv4_addr(vip, port_)) {
      TBSYS_LOG(ERROR, "nameserver vip address invalid, ip:%d, port:%d", local_ip, port_);
      ret = OB_ERROR;
  ...
  if (OB_SUCCESS == ret) {
    if (OB_SUCCESS != (ret = ms_list_task_.init(
      ...
      TBSYS_LOG(ERROR, "init ms list failt, ret: [%d]", ret);
               ...
               (ret = config_mgr_.init(name_server_.get_ms_provider(), client_manager, timer_))) {
...
int NameServerWorker::set_io_thread_count(int io_thread_count) {
  int ret = OB_SUCCESS;
...
int NameServerWorker::start_as_master() {
  ...
  name_server_.init_boot_state();
  ret = log_manager_.init(&name_server_, &slave_mgr_, &get_name_server().get_self());
  if (ret == OB_SUCCESS) {
    // try to replay log
  ...
  if (ret == OB_SUCCESS) {
    ...
    name_server_.start_merge_check();
  }

  if (ret == OB_SUCCESS) {
    TBSYS_LOG(INFO, "[NOTICE] master start step4");
    name_server_.reset_hb_time();
  }
  ...
  if (ret == OB_SUCCESS) {
    ...
    name_server_.start_master_nameserver();
    check_thread_.start();
...
int NameServerWorker::start_as_slave() {
  ...
  if (OB_SUCCESS == err) {
    ...
    //read_thread_queue_.start();
    err = log_manager_.init(&name_server_, &slave_mgr_, &get_name_server().get_self());
  }
  ...
  if (err == OB_SUCCESS) {
    ...
    if (err == OB_SUCCESS) {
      ...
      fetch_thread_.add_ckpt_ext(NameServer::LOAD_DATA_EXT); // add load data check point
  ...
  if (err == OB_SUCCESS) {
    ...
    if (ObRoleMgr::SWITCHING == role_mgr_.get_state()) {
      ...
      write_thread_queue_.start();
      if (OB_SUCCESS == err) {
        err = name_server_.init_boot_state();
      }
      ...
      if (OB_SUCCESS == err) {
        ...
        while (OB_SUCCESS != err && ObRoleMgr::STOP != role_mgr_.get_state()
               && ObRoleMgr::ERROR != role_mgr_.get_state()) {
          if (OB_SUCCESS != (err = name_server_.refresh_new_schema(count))) {
            TBSYS_LOG(WARN, "name_server.refresh_new_schema(count)=>%d", err);
          } else if (OB_SUCCESS != (err = name_server_.get_last_frozen_version_from_ups(false))) {
      ...
      if (OB_SUCCESS != err) {
        ...
        name_server_.after_switch_to_master();
...
int NameServerWorker::send_obi_role(common::ObiRole obi_role) {
  int ret = OB_SUCCESS;
  TBSYS_LOG(INFO, "send obi_role to slave nameserver");
...
int NameServerWorker::get_boot_state_from_master() {
  int ret = OB_SUCCESS;
  bool boot_ok = false;
  const static int SLEEP_US_WHEN_INIT = 2000 * 1000; // 2s
  while (true) {
    ...
    if (OB_SUCCESS != ret) {
      ...
      ret = name_server_.init_first_meta();
      if (OB_SUCCESS != ret) {
...
void NameServerWorker::destroy() {
  name_server_.grant_eternal_ups_lease();
...
int NameServerWorker::submit_restart_task() {
  int ret = OB_SUCCESS;
...
int NameServerWorker::submit_delete_tablets_task(const common::ObTabletReportInfoList& delete_list) {
  int ret = OB_SUCCESS;
  ThreadSpecificBuffer::Buffer* my_buffer = my_thread_buffer.get_buffer();
  if (NULL == my_buffer) {
    TBSYS_LOG(ERROR, "alloc thread buffer fail");
...
int NameServerWorker::handlePacket(ObPacket* packet) {
  int ret = OB_SUCCESS;
  bool ps = true;
...
int NameServerWorker::handleBatchPacket(ObPacketQueue& packetQueue) {
  UNUSED(packetQueue);
  TBSYS_LOG(ERROR, "you should not reach this, not supporrted");
  return OB_SUCCESS;
}

bool NameServerWorker::handlePacketQueue(ObPacket* packet, void* args) {
  bool ret = true;
          ...
          if (my_buffer != NULL) {
            ...
            if ((void*)WRITE_THREAD_FLAG == args) {
              ...
              case OB_CS_DELETE_TABLE_DONE:
                ...
                break;
              case OB_CS_LOAD_BYPASS_SSTABLE_DONE:
                return_code = ns_cs_load_bypass_sstable_done(version, *in_buf, req, channel_id, thread_buff);
                break;
              case OB_SET_OBI_ROLE_TO_SLAVE:
            ...
            // read queue
              ...
              case OB_RS_ADMIN:
                ...
                break;
              case OB_RS_CHECK_ROOTTABLE:
                return_code = ns_check_root_table(version, *in_buf, req, channel_id, thread_buff);
                break;
              case OB_RS_DUMP_CS_TABLET_INFO:
                return_code = ns_dump_cs_tablet_info(version, *in_buf, req, channel_id, thread_buff);
                break;
              case OB_RS_STAT:
...
int NameServerWorker::schedule_after_restart_task(const int64_t delay, bool repeat /*=false*/) {
...
int NameServerWorker::rt_get_update_server_info(const int32_t version, ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret && OB_SUCCESS == result_msg.result_code_) {
    found_server = name_server_.get_update_server_info(use_inner_port);
  }
...
int NameServerWorker::rt_scan(const int32_t version, ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret && OB_SUCCESS == result_msg.result_code_) {
    result_msg.result_code_ = name_server_.find_root_table_range(scan_param_in, *scanner);
  }
...
int NameServerWorker::rt_sql_scan(const int32_t version,
  ...
  if (OB_SUCCESS == rc.result_code_) {
    ...
    int64_t table_count = name_server_.get_table_count();
    OB_STAT_SET(ROOTSERVER, INDEX_ALL_TABLE_COUNT, table_count);
    int64_t tablet_count = 0;
    int64_t row_count = 0;
    int64_t data_size = 0;
    name_server_.get_tablet_info(tablet_count, row_count, data_size);
...
int NameServerWorker::rt_get(const int32_t version, ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret && OB_SUCCESS == result_msg.result_code_) {
    if ((*get_param)[0]->table_id_ == OB_ALL_SERVER_STAT_TID) {
      result_msg.result_code_ = name_server_.find_monitor_table_key(*get_param, *scanner);
    } else if ((*get_param)[0]->table_id_ == OB_ALL_SERVER_SESSION_TID) {
      result_msg.result_code_ = name_server_.find_session_table_key(*get_param, *scanner);
    } else if ((*get_param)[0]->table_id_ == OB_ALL_STATEMENT_TID) {
      result_msg.result_code_ = name_server_.find_statement_table_key(*get_param, *scanner);
    } else {
      result_msg.result_code_ = name_server_.find_root_table_key(*get_param, *scanner);
    }
  }
...
int NameServerWorker::rt_fetch_schema(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret && OB_SUCCESS == result_msg.result_code_) {
    if (OB_SUCCESS != (ret = name_server_.get_schema(false, get_only_core_tables, *schema))) {
...
int NameServerWorker::rt_after_restart(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == result_msg.result_code_ && OB_SUCCESS == ret) {
    result_msg.result_code_ = name_server_.after_restart();
    if (OB_SUCCESS != result_msg.result_code_) {
      TBSYS_LOG(WARN, "fail to restart nameserver. ret=%d", ret);
    } else {
      TBSYS_LOG(INFO, "restart nameserver ok~!");
    }
  }
  return ret;
}
...
int NameServerWorker::rt_waiting_job_done(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret) {
    result_msg.result_code_ = name_server_.waiting_job_done(server, frozen_mem_version);
  }
  if (OB_SUCCESS == ret) {
    ret = result_msg.serialize(out_buff.get_data(), out_buff.get_capacity(), out_buff.get_position());
    if (ret != OB_SUCCESS) {
...
int NameServerWorker::rt_delete_tablets(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret) {
    ...
    if (OB_SUCCESS == ret) {
      ret = name_server_.get_cs_info(out_server_manager);
      if (OB_SUCCESS != ret) {
...
int NameServerWorker::rt_cs_delete_tablets(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret) {
    ret = name_server_.delete_replicas(false, server, tablet_list);
    if (ret != OB_SUCCESS) {
...
int NameServerWorker::rt_register(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret) {
    ...
    result_msg.result_code_ = name_server_.regist_chunk_server(server, server_version, status);
  }
  if (OB_SUCCESS == ret) {
    ret = result_msg.serialize(out_buff.get_data(), out_buff.get_capacity(), out_buff.get_position());
    if (ret != OB_SUCCESS) {
...
int NameServerWorker::rt_register_ms(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret) {
    ...
    result_msg.result_code_ = name_server_.regist_merge_server(server, sql_port, lms, server_version);
  }
...
int NameServerWorker::rt_migrate_over(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret) {
    result_msg.result_code_ = name_server_.migrate_over(result_msg.result_code_,
...
int NameServerWorker::rt_heartbeat(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret && OB_SUCCESS == result_msg.result_code_) {
    result_msg.result_code_ = name_server_.receive_hb(server, server.get_port(), false, role);
  }
...
int NameServerWorker::rt_heartbeat_ms(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if ((OB_SUCCESS == ret) && (OB_SUCCESS == result_msg.result_code_)) {
    result_msg.result_code_ = name_server_.receive_hb(server, sql_port, is_listen_ms, role);
  }
  easy_request_wakeup(req);
  return ret;
}
...
int NameServerWorker::rt_check_tablet_merged(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == err && OB_SUCCESS == result_msg.result_code_) {
    result_msg.result_code_ = name_server_.check_tablet_version(tablet_version, 0, is_merged);
    if (OB_SUCCESS != result_msg.result_code_) {
...
int NameServerWorker::rt_update_server_report_freeze(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (ret == OB_SUCCESS) {
    name_server_.report_frozen_memtable(frozen_version, 0, false);
  }
  ...
  if (ret == OB_SUCCESS) {
    ...
    name_server_.get_max_tablet_version(rt_version);
    name_server_.receive_new_frozen_version(rt_version, frozen_version, 0, false);
  }
...
int NameServerWorker::rt_set_obi_role_to_slave(const int32_t version, common::ObDataBuffer& in_buff, easy_request_t* req, const uint32_t channel_id, common::ObDataBuffer& out_buff) {
  int ret = OB_SUCCESS;
  static const int MY_VERSION = 1;
  UNUSED(version);
  common::ObResultCode result_msg;
...
int NameServerWorker::rt_get_boot_state(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  result_msg.result_code_ = OB_SUCCESS;
  int ret = OB_SUCCESS;
  bool boot_ok = (name_server_.get_boot()->is_boot_ok());
...
int NameServerWorker::rt_force_cs_to_report(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  common::ObResultCode result_msg;
  result_msg.result_code_ = OB_SUCCESS;
  ret = name_server_.request_cs_report_tablet();
  if (OB_SUCCESS != ret) {
...
int NameServerWorker::rt_set_obi_role(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS != (ret = role.deserialize(in_buff.get_data(), in_buff.get_capacity(), in_buff.get_position()))) {
    ...
    ret = name_server_.set_obi_role(role);
    name_server_.commit_task(OBI_ROLE_CHANGE, OB_ROOTSERVER, rt_master_, rt_master_.get_port(), "nameserver",
...
int NameServerWorker::rt_get_last_frozen_version(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS != (ret = result_msg.serialize(out_buff.get_data(), out_buff.get_capacity(),
    ...
    TBSYS_LOG(WARN, "serialize error, err=%d", ret);
                                                             ...
                                                             out_buff.get_position(), name_server_.get_last_frozen_version()))) {
...
int NameServerWorker::ns_check_root_table(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == err) {
    is_integrity = name_server_.check_root_table(cs);
  }
  if (OB_SUCCESS == err) {
    err = result_msg.serialize(out_buff.get_data(), out_buff.get_capacity(), out_buff.get_position());
    if (OB_SUCCESS != err) {
...
int NameServerWorker::ns_dump_cs_tablet_info(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == err) {
    name_server_.dump_cs_tablet_info(cs, tablet_num);
  }
  if (OB_SUCCESS == err) {
    err = result_msg.serialize(out_buff.get_data(), out_buff.get_capacity(), out_buff.get_position());
    if (OB_SUCCESS != err) {
...
int NameServerWorker::do_admin_with_return(int admin_cmd) {
  int ret = OB_SUCCESS;
  switch (admin_cmd) {
...
int NameServerWorker::do_admin_without_return(int admin_cmd) {
  int64_t count = 0;
  int ret = OB_SUCCESS;
  switch (admin_cmd) {
  case OB_RS_ADMIN_BOOT_RECOVER:
    TBSYS_LOG(INFO, "start to boot recover");
    ret = name_server_.boot_recover();
    break;
  case OB_RS_ADMIN_DUMP_ROOT_TABLE:
    name_server_.dump_root_table();
    break;
  ...
  case OB_RS_ADMIN_DUMP_MIGRATE_INFO:
    ...
    break;
  case OB_RS_ADMIN_REFRESH_SCHEMA:
    name_server_.renew_user_schema(count);
    break;
  default:
...
int NameServerWorker::rt_get_master_ups_config(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  result_msg.result_code_ = name_server_.get_master_ups_config(master_master_ups_read_percent,
...
int NameServerWorker::rt_ups_heartbeat_resp(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (msg.MY_VERSION != version) {
    ...
    ret = name_server_.receive_ups_heartbeat_resp(msg.addr_, ups_status, msg.obi_role_);
  }
  // no response
  easy_request_wakeup(req);
  return ret;
...
int NameServerWorker::rt_get_ups(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (MY_VERSION != version) {
    ...
    ret = name_server_.get_ups_list(ups_list);
  }
...
int NameServerWorker::rt_ups_register(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (msg.MY_VERSION != version) {
    ...
    ret = name_server_.register_ups(msg.addr_, msg.inner_port_, msg.log_seq_num_, msg.lease_, msg.server_version_);
  }
  // send response
  ObResultCode res;
  res.result_code_ = ret;
...
int NameServerWorker::rt_set_master_ups_config(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (MY_VERSION != version) {
    ...
    common::ObResultCode res;
    res.result_code_ = name_server_.set_ups_config(read_master_master_ups_percentage,
...
int NameServerWorker::rt_ups_slave_failure(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (msg.MY_VERSION != version) {
    ...
    ret = name_server_.ups_slave_failure(msg.my_addr_, msg.slave_addr_);
  }
  // send response
  static const int MY_VERSION = 1;
  common::ObResultCode res;
...
int NameServerWorker::rt_change_ups_master(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (MY_VERSION != version) {
    ...
    ret = name_server_.change_ups_master(ups_addr, did_force);
  }
  // send response
  common::ObResultCode res;
  res.result_code_ = ret;
...
int NameServerWorker::rt_get_row_checksum(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (MY_VERSION != version) {
    ...
    TBSYS_LOG(WARN, "failed to deserialize");
    ret = OB_INVALID_ARGUMENT;
  } else if (OB_SUCCESS != (ret = name_server_.get_row_checksum(tablet_version, table_id, row_checksum)) && OB_ENTRY_NOT_EXIST != ret) {
...
int NameServerWorker::rt_get_proxy_list(const int32_t version, common::ObDataBuffer& in_buff, easy_request_t* req, const uint32_t channel_id, common::ObDataBuffer& out_buff) {
  int ret = OB_SUCCESS;
  UNUSED(in_buff);
  static const int MY_VERSION = 1;
  if (MY_VERSION != version) {
  ...
  if (OB_SUCCESS != (ret = res.serialize(out_buff.get_data(), out_buff.get_capacity(), out_buff.get_position()))) {
    TBSYS_LOG(WARN, "serialize error, err=%d", ret);
  } else if (OB_SUCCESS == res.result_code_ && OB_SUCCESS != (ret = name_server_.serialize_proxy_list(
      out_buff.get_data(), out_buff.get_capacity(), out_buff.get_position()))) {
    TBSYS_LOG(WARN, "serialize error, err=%d", ret);
  } else {
...
int NameServerWorker::rt_cs_import_tablets(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (MY_VERSION != version) {
    ...
    ret = name_server_.cs_import_tablets(table_id, tablet_version);
  }
  common::ObResultCode res;
  res.result_code_ = ret;
  if (OB_SUCCESS != (ret = res.serialize(out_buff.get_data(), out_buff.get_capacity(), out_buff.get_position()))) {
...
int NameServerWorker::rt_restart_cs(const int32_t version, ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret) {
    if (!is_restart_all) {
      if (cancel) {
        ret = name_server_.cancel_shutdown_cs(servers, RESTART);
      } else {
        ret = name_server_.shutdown_cs(servers, RESTART);
      }
    } else {
      if (cancel) {
        name_server_.cancel_restart_all_cs();
      } else {
        name_server_.restart_all_cs();
      }
    }
  }
  common::ObResultCode res;
...
int NameServerWorker::rt_shutdown_cs(const int32_t version, ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret) {
    if (cancel) {
      ret = name_server_.cancel_shutdown_cs(servers, SHUTDOWN);
    } else {
      ret = name_server_.shutdown_cs(servers, SHUTDOWN);
    }
  }
  common::ObResultCode res;
  res.result_code_ = ret;
...
common::ObClientManager* NameServerWorker::get_client_manager() {
  return &client_manager;
}

int64_t NameServerWorker::get_network_timeout() {
  return config_.network_timeout;
}

common::ObServer NameServerWorker::get_ns_master() {
  return rt_master_;
}

common::ThreadSpecificBuffer* NameServerWorker::get_thread_buffer() {
...
int NameServerWorker::rt_split_tablet(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == err && OB_SUCCESS == result_msg.result_code_) {
    int64_t rt_version = 0;
    name_server_.get_max_tablet_version(rt_version);
    result_msg.result_code_ = name_server_.receive_new_frozen_version(
...
int NameServerWorker::rt_create_table(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  TableSchema tschema;
  ObString user_name;
  common::ObiRole::Role role = name_server_.get_obi_role().get_role();
  if (MY_VERSION != version) {
    ...
    if (OB_SUCCESS != (ret = serialization::decode_bool(in_buff.get_data(),
      ...
      TBSYS_LOG(WARN, "failed to deserialize, err=%d", ret);
                                                        ...
                                                        in_buff.get_capacity(), in_buff.get_position()))) {
      TBSYS_LOG(WARN, "failed to deserialize, err=%d", ret);
    } else if (OB_SUCCESS != (ret = name_server_.create_table(if_not_exists, tschema))) {
...
int NameServerWorker::rt_alter_table(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (MY_VERSION != version) {
    ...
    if (OB_SUCCESS != (ret = tschema.deserialize(in_buff.get_data(), in_buff.get_capacity(),
                                                 in_buff.get_position()))) {
      TBSYS_LOG(WARN, "failed to deserialize, err=%d", ret);
    } else if (OB_SUCCESS != (ret = name_server_.alter_table(tschema))) {
  ...
  if (MY_VERSION != version) {
    ...
    if (OB_SUCCESS != (ret = serialization::decode_bool(in_buff.get_data(),
      ...
      TBSYS_LOG(WARN, "failed to deserialize, err=%d", ret);
                                                       ...
                                                       in_buff.get_capacity(), in_buff.get_position()))) {
      TBSYS_LOG(WARN, "failed to deserialize, err=%d", ret);
    } else if (OB_SUCCESS != (ret = name_server_.drop_tables(if_exists, tables))) {
...
int NameServerWorker::rt_execute_sql(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  } else if (NULL == (server_mgr = &name_server_.get_server_manager())) {
    TBSYS_LOG(WARN, "server manager is not inited right now");
    ret = OB_NOT_INIT;
  } else {
    bool table_exist = false;
    if (OB_SUCCESS != (ret = name_server_.check_table_exist(table_name, table_exist))) {
...
int NameServerWorker::rt_handle_trigger_event(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  TBSYS_LOG(INFO, "I got a trigger message. value=%.*s,%ld,%ld. my role=%d",
            ...
            static_cast<int>(name_server_.get_obi_role().get_role()));
  switch (msg.type) {
  case REFRESH_NEW_SCHEMA_TRIGGER: {
    int64_t count = 0;
    if (role != common::ObiRole::MASTER) {
      ret = name_server_.renew_user_schema(count);
    }
  ...
  case CREATE_TABLE_TRIGGER: {
    if (role != common::ObiRole::MASTER) {
      if (OB_SUCCESS != (ret = name_server_.trigger_create_table(msg.param))) {
  ...
  case DROP_TABLE_TRIGGER: {
    if (role != common::ObiRole::MASTER) {
      if (OB_SUCCESS != (ret = name_server_.trigger_drop_table(msg.param))) {
...
int NameServerWorker::rt_get_master_obi_rs(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS != (ret = res.serialize(out_buff.get_data(),
    ...
    if (role == common::ObiRole::MASTER) {
      ...
      ObServer master_rs;
      if (OB_SUCCESS != config_.get_master_name_server(master_rs)) {
        TBSYS_LOG(ERROR, "Get master name server error, ret: [%d]", ret);
...
int NameServerWorker::rt_check_task_process(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  UNUSED(in_buff);
  UNUSED(out_buff);
  name_server_.check_bypass_process();
  if (OB_SUCCESS != ret) {
...
int NameServerWorker::rt_prepare_bypass_process(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret && OB_SUCCESS == result_msg.result_code_) {
    ...
    result_msg.result_code_ = name_server_.prepare_bypass_process(table_name_id);
    if (OB_SUCCESS != result_msg.result_code_) {
...
int NameServerWorker::ns_cs_load_bypass_sstable_done(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret && OB_SUCCESS == result_msg.result_code_) {
    result_msg.result_code_ = name_server_.cs_load_sstable_done(cs, table_list, is_load_succ);
    if (OB_SUCCESS != result_msg.result_code_) {
...
int NameServerWorker::rt_cs_delete_table_done(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret && OB_SUCCESS == result_msg.result_code_) {
    result_msg.result_code_ = name_server_.cs_delete_table_done(cs, table_id, is_delete_succ);
    if (OB_SUCCESS != result_msg.result_code_) {
      TBSYS_LOG(WARN, "fail to do bypass proces. cs_addr=%s, ret=%d", inet_ntoa_r(addr), ret);
    }
  }
...
int NameServerWorker::rt_start_bypass_process(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret && OB_SUCCESS == result_msg.result_code_) {
    result_msg.result_code_ = name_server_.start_bypass_process(table_name_id);
    if (OB_SUCCESS != result_msg.result_code_) {
...
int NameServerWorker::submit_check_task_process() {
  int ret = OB_SUCCESS;
  ThreadSpecificBuffer::Buffer* my_buffer = my_thread_buffer.get_buffer();
  if (NULL == my_buffer) {
    TBSYS_LOG(ERROR, "alloc thread buffer fail");
...
int NameServerWorker::rt_write_schema_to_file(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret && OB_SUCCESS == result_msg.result_code_) {
    if (OB_SUCCESS != (ret = name_server_.write_schema_to_file())) {
...
int NameServerWorker::rt_change_table_id(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (MY_VERSION != version) {
    ...
    ret = name_server_.change_table_id(table_id);
  }
  // send response
  common::ObResultCode res;
  res.result_code_ = ret;
...
int NameServerWorker::rt_start_import(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  } else if (OB_SUCCESS != (ret = name_server_.start_import(table_name, table_id, uri))) {
...
int NameServerWorker::rt_import(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (MY_VERSION != version) {
    ...
    TBSYS_LOG(ERROR, "load_data is not enabled, cant load table %.*s %lu",
              table_name.length(), table_name.ptr(), table_id);
  } else if (OB_SUCCESS != (ret = name_server_.import(table_name, table_id, uri, start_time))) {
    TBSYS_LOG(WARN, "failed to import table_name=%.*s table_id=%lu, uri=%.*s, ret=%d",
              table_name.length(), table_name.ptr(), table_id, uri.length(), uri.ptr(), ret);
  }
...
int NameServerWorker::rt_start_kill_import(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret) {
    ret = name_server_.start_kill_import(table_name, table_id);
    if (OB_SUCCESS != ret) {
...
int NameServerWorker::rt_kill_import(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret) {
    ret = name_server_.kill_import(table_name, table_id);
    if (OB_SUCCESS != ret) {
      TBSYS_LOG(ERROR, "failed to kill import table_name=%.*s table_id=%lu", table_name.length(),
                table_name.ptr(), table_id);
    }
...
int NameServerWorker::rt_get_import_status(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret) {
    ret = name_server_.get_import_status(table_name, table_id, status);
    if (OB_SUCCESS != ret) {
...
int NameServerWorker::rt_set_import_status(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (OB_SUCCESS == ret) {
    ...
    ret = name_server_.set_import_status(table_name, table_id, status);
    if (OB_SUCCESS != ret) {
      TBSYS_LOG(ERROR, "failed to check import status table_name=%.*s table_id=%lu, ret=%d", table_name.length(),
                table_name.ptr(), table_id, ret);
    }
...
int NameServerWorker::rt_force_create_table(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (MY_VERSION != version) {
    TBSYS_LOG(WARN, "un-supported rpc version=%d", version);
    res.result_code_ = OB_ERROR_FUNC_VERSION;
  } else if (name_server_.get_obi_role().get_role() == common::ObiRole::MASTER) {
    res.result_code_ = OB_OP_NOT_ALLOW;
    TBSYS_LOG(WARN, "master instance can't force to create table. role=%s", name_server_.get_obi_role().get_role_str());
  } else {
    ...
    } else if (OB_SUCCESS != (ret = name_server_.force_create_table(table_id))) {
...
int NameServerWorker::rt_force_drop_table(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (MY_VERSION != version) {
    ...
    res.result_code_ = OB_OP_NOT_ALLOW;
    TBSYS_LOG(WARN, "master instance can't force to drop table. role=%s", name_server_.get_obi_role().get_role_str());
  } else {
    if (OB_SUCCESS != (ret = serialization::decode_vi64(in_buff.get_data(),
                                                        in_buff.get_capacity(), in_buff.get_position(), (int64_t*)&table_id))) {
      TBSYS_LOG(WARN, "failed to deserialize table_id, err=%d", ret);
    } else if (OB_SUCCESS != (ret = name_server_.force_drop_table(table_id))) {
...
int NameServerWorker::rt_notify_switch_schema(const int32_t version, common::ObDataBuffer& in_buff,
  ...
  if (MY_VERSION != version) {
    TBSYS_LOG(WARN, "un-supported rpc version=%d", version);
    res.result_code_ = OB_ERROR_FUNC_VERSION;
  } else if (OB_SUCCESS != (ret = name_server_.notify_switch_schema(only_core_tables, force_update))) {
