 ...
 *
 * ob_rs_stress.cc
 * nameserver stress testing tool
 * Authors:
...
inline MyArguments::MyArguments() {
  ns_host = DEFAULT_RS_HOST;
  ns_port = DEFAULT_RS_PORT;
  batch_count = 0;
...
const char* MyMain::PROG_NAME = "ns_stress";
MyMain::MyMain() {
}
...
void MyMain::usage() {
  printf("\nUsage: %s -r <nameserver_ip> -p <nameserver_port> <options>\n", PROG_NAME);
...
ObRsStressRunnable::ObRsStressRunnable(ObRootRpcStub& rpc_stub, ObServer& rs, MyArguments& args)
  : rpc_stub_(rpc_stub), ns_(rs), args_(args) {
}
...
void ObRsStressRunnable::run(tbsys::CThread* thread, void* arg) {
  ...
  for (int i = 0; i < args_.batch_count; ++i) {
    if (OB_SUCCESS != (ret = rpc_stub_.get_obi_role(ns_, args_.timeout_us, obi_role))) {
...
int main(int argc, char* argv[]) {
  ...
  MyMain my_main;
  MyArguments args;
  TBSYS_LOGGER.setFileName("ns_stress.log", true);
