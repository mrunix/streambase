...
ObBootstrap::ObBootstrap(NameServer& name_server): name_server_(name_server), log_worker_(NULL) {
  core_table_count_ = 0;
  sys_table_count_ = 0;
  ini_table_count_ = 0;
}
...
void ObBootstrap::set_log_worker(NameServerLogWorker* log_worker) {
...
int ObBootstrap::create_core_table(const uint64_t table_id, ObServerArray& first_tablet_entry_cs) {
  ...
  int ret = name_server_.get_table_schema(table_id, table_schema);
  if (ret != OB_SUCCESS) {
    TBSYS_LOG(WARN, "fail to get talbe schema. table_id=%lu err=%d", table_id, ret);
  } else {
    ret = name_server_.create_empty_tablet(table_schema, first_tablet_entry_cs);
    if (OB_SUCCESS != ret) {
...
int ObBootstrap::create_sys_table(TableSchema& table_schema) {
  int ret = OB_SUCCESS;
  const ObiRole& role = name_server_.get_obi_role();
  ...
  if (OB_SUCCESS == ret) {
    ...
    ret = name_server_.create_empty_tablet(table_schema, cs);
    if (OB_SUCCESS != ret) {
...
int ObBootstrap::init_schema_service(void) {
  int ret = schema_service_.init(name_server_.schema_service_scan_helper_, true);
  if (OB_SUCCESS != ret) {
...
int ObBootstrap::bootstrap_ini_tables() {
  ...
  const ObiRole& role = name_server_.get_obi_role();
  if (NULL == (schema_manager = name_server_.get_ini_schema())) {
    ret = OB_ERROR;
...
int ObBootstrap::init_all_cluster() {
  ...
  if (OB_SUCCESS != (
    ...
    for (int64_t i = 0; i < config.retry_times; i++) {
      ...
      } else if (OB_SUCCESS != name_server_.get_rpc_stub().execute_sql(server, sql, NETWORK_TIMEOUT)) {
  ...
  if (OB_SUCCESS == ret) {
    ...
    name_server_.commit_task(SERVER_ONLINE, OB_ROOTSERVER, rs, 0, server_version);
  }
  return ret;
}
...
int ObBootstrap::init_all_sys_config_stat() {
  int ret = OB_SUCCESS;
  ObConfigManager* config_mgr = name_server_.get_config_mgr();
  int64_t version = tbsys::CTimeUtil::getTime();
  if (NULL == config_mgr) {
    ret = OB_NOT_INIT;
  } else {
    name_server_.log_worker_->got_config_version(version);
...
int ObBootstrap::init_users() {
  ...
  if (OB_SUCCESS != (ret = acc.init(name_server_.schema_service_scan_helper_))) {
  ...
  if (OB_SUCCESS != (ret = acc.init(name_server_.schema_service_scan_helper_))) {
    ...
    if (OB_SUCCESS == ret) {
      ...
      if (OB_SUCCESS != (ret = acc.insert(OB_ALL_SYS_STAT_TABLE_NAME, rowkey,
        ...
        name_server_.set_privilege_version(privilege_version);
...
#define INSERT_ALL_SYS_PARAM_ROW(ret, acc, cname, itype_value, cvalue, cinfo)  \
  ...
  { \
    ...
    INSERT_ALL_SYS_PARAM_ROW(
      ...
      "ob_app_name",
      ObIntType,
      /* name_server_.config_.client_config_.app_name_, */
      "",
      "app name");
    INSERT_ALL_SYS_PARAM_ROW(
      ret,
...
int ObBootstrap::init_meta_file(const ObArray<ObServer>& created_cs) {
  ...
  if (OB_SUCCESS == ret) {
    if (OB_SUCCESS != (ret = name_server_.get_first_meta()->init(first_meta_row))) {
