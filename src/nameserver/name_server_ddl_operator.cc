...
NameServerDDLOperator::NameServerDDLOperator(): schema_client_(NULL), name_server_(NULL) {
}

NameServerDDLOperator::~NameServerDDLOperator() {
}

void NameServerDDLOperator::init(NameServer* server, ObSchemaService* impl) {
  name_server_ = server;
  schema_client_ = impl;
}

int NameServerDDLOperator::create_table(const TableSchema& table_schema) {
  int ret = OB_SUCCESS;
  if (!check_inner_stat()) {
    ret = OB_ERROR;
...
int NameServerDDLOperator::create_empty_tablet(const TableSchema& table_schema) {
  ObArray<ObServer> cs_list;
  int ret = name_server_->create_empty_tablet(const_cast<TableSchema&>(table_schema), cs_list);
  if (ret != OB_SUCCESS) {
...
bool NameServerDDLOperator::insert_schema_table(const TableSchema& table_schema) {
  bool succ = false;
...
int NameServerDDLOperator::allocate_table_id(TableSchema& table_schema) {
  int ret = OB_SUCCESS;
  uint64_t table_id = 0;
...
int NameServerDDLOperator::update_max_table_id(const uint64_t table_id) {
  int ret = OB_SUCCESS;
  uint64_t max_id = 0;
...
int NameServerDDLOperator::drop_table(const common::ObString& table_name) {
  int ret = OB_SUCCESS;
  uint64_t table_id = 0;
  if (!check_inner_stat()) {
    ret = OB_ERROR;
...
int NameServerDDLOperator::delete_root_table(const uint64_t table_id) {
  ...
  if (ret != OB_SUCCESS) {
    ...
    ret = name_server_->delete_tables(false, list);
    if (ret != OB_SUCCESS) {
...
bool NameServerDDLOperator::delete_schema_table(const common::ObString& table_name, uint64_t& table_id) {
  bool succ = false;
  tbsys::CThreadGuard lock(&mutex_lock_);
...
int NameServerDDLOperator::alter_table(AlterTableSchema& table_schema) {
  int ret = OB_SUCCESS;
  if (!check_inner_stat()) {
    ret = OB_ERROR;
    TBSYS_LOG(WARN, "check inner stat failed");
...
int NameServerDDLOperator::alter_table_schema(const TableSchema& old_schema, AlterTableSchema& alter_schema) {
  int ret = OB_SUCCESS;
...
int NameServerDDLOperator::modify_table_id(common::TableSchema& table_schema, const int64_t new_table_id) {
  int ret = OB_SUCCESS;
  if (!check_inner_stat()) {
    ret = OB_ERROR;
    TBSYS_LOG(WARN, "check inner stat failed");
