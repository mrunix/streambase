...
NameServerMonitorTable::NameServerMonitorTable() {
  cs_manager_ = NULL;
  ups_manager_ = NULL;
}
...
int NameServerMonitorTable::get(const ObRowkey& rowkey, ObScanner& scanner) {
  int pos = -1;
  int ret = OB_SUCCESS;
...
int NameServerMonitorTable::get_ms_only(const ObRowkey& rowkey, ObScanner& scanner) {
  int pos = -1;
  int ret = OB_SUCCESS;
  // add all type of server for in memory table
  int64_t count = 0;
...
int NameServerMonitorTable::insert_all_meta_servers(ServerVector& servers) const {
  ObClusterServer server;
  server.role = OB_ROOTSERVER;
  server.addr = nameserver_vip_;
  int ret = servers.push_back(server);
  if (ret != OB_SUCCESS) {
    TBSYS_LOG(WARN, "add name server failed:ret[%d]", ret);
  } else {
    TBSYS_LOG(DEBUG, "add name server succ:vip[%s]", server.addr.to_cstring());
  }
  return ret;
}

int NameServerMonitorTable::insert_all_query_servers(ServerVector& servers) const {
  int ret = OB_SUCCESS;
  int64_t cs_count = 0;
  int64_t ms_count = 0;
  ObClusterServer server;
...
int NameServerMonitorTable::insert_all_merge_servers(ServerVector& servers) const {
  int ret = OB_SUCCESS;
  int64_t ms_count = 0;
  ObClusterServer server;
  const ObServerStatus* it = NULL;
...
int NameServerMonitorTable::insert_all_update_servers(ServerVector& servers) const {
  int ret = OB_SUCCESS;
  int64_t count = 0;
...
void NameServerMonitorTable::init_all_servers(ServerVector& servers, int64_t& count) const {
  servers.clear();
  int ret = insert_all_meta_servers(servers);
  if (ret != OB_SUCCESS) {
    TBSYS_LOG(WARN, "insert all name server failed:ret[%d]", ret);
  }
...
void NameServerMonitorTable::init_all_ms(ServerVector& servers, int64_t& count) const {
  servers.clear();
...
void NameServerMonitorTable::sort_all_servers(ServerVector& servers) const {
  std::sort(servers.begin(), servers.end(), Compare());
}

int NameServerMonitorTable::find_first_tablet(const ObRowkey& rowkey, const ServerVector& servers) const {
  int ret = -1;
...
int NameServerMonitorTable::add_normal_row(const int64_t index, const ServerVector& servers, ObScanner& result) {
  int ret = OB_SUCCESS;
...
// ugly fake first row
int NameServerMonitorTable::add_first_row(ObScanner& result) {
  ObCellInfo cell;
  cell.table_name_ = TNAME;
...
int NameServerMonitorTable::add_server_columns(const ObServer& server, ObCellInfo& cell, ObScanner& result) {
...
int NameServerMonitorTable::add_last_row(const ServerVector& servers, ObScanner& result) {
  int ret = OB_SUCCESS;
...
int NameServerMonitorTable::convert_server_2_rowkey(const ObClusterServer& server, ObRowkey& rowkey) {
  int ret = OB_SUCCESS;
...
int NameServerMonitorTable::convert_rowkey_2_server(const ObRowkey& rowkey, ObClusterServer& server) const {
  int ret = OB_SUCCESS;
...
void NameServerMonitorTable::convert_role(const ObRole& server_role, ObString& role) {
  role = ObString::make_string(print_role(server_role));
}

// convert string to id role
int NameServerMonitorTable::convert_role(const ObObj& obj, ObRole& role) const {
  int ret = OB_SUCCESS;
  if (obj.is_min_value()) {
    role = OB_INVALID;
  } else {
...
int NameServerMonitorTable::convert_addr(const ObServer& server, ObString& addr) {
  int ret = OB_SUCCESS;
...
int NameServerMonitorTable::convert_addr(const ObObj& ip, const ObObj& port, ObServer& server) const {
  int ret = OB_SUCCESS;
...
void NameServerMonitorTable::print_info(const bool sort) const {
  ServerVector servers;
  int64_t count = 0;
  init_all_servers(servers, count);
  if (true == sort) {
