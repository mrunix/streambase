...
#include "name_server_util.h"
#include "name_server_bootstrap.h"
#include "name_server_ms_provider.h"
#include "name_server_monitor_table.h"
...
const char* NameServer::LOAD_DATA_EXT = "load_data";
...
const ObChunkServerManager& NameServer::get_server_manager() const {
  return server_manager_;
}

NameServerAsyncTaskQueue* NameServer::get_task_queue() {
  return &seq_task_queue_;
}

ObFirstTabletEntryMeta* NameServer::get_first_meta() {
  return first_meta_;
}

ObConfigManager* NameServer::get_config_mgr() {
...
bool NameServer::init(const int64_t now, NameServerWorker* worker) {
  bool res = false;
  UNUSED(now);
  if (NULL == worker) {
    ...
    if (NULL == (local_schema_manager_ = OB_NEW(ObSchemaManagerV2, ObModIds::OB_RS_SCHEMA_MANAGER, now))) {
      ...
      TBSYS_LOG(ERROR, "no memory");
                        ...
                        = new(std::nothrow) ObFirstTabletEntryMeta((int32_t)config_.read_queue_size,
                            get_file_path(config_.ns_data_dir,
      ...
      TBSYS_LOG(ERROR, "no memory");
    } else if (NULL == (balancer_thread_ = new(std::nothrow) NameServerBalancerRunnable(
...
void NameServer::after_switch_to_master() {
...
int NameServer::init_boot_state() {
  int ret = OB_SUCCESS;
  ...
  if (OB_INIT_TWICE == ret || OB_SUCCESS == ret) {
    TBSYS_LOG(INFO, "nameserver alreay bootstrap, set boot ok.");
...
int NameServer::start_master_nameserver() {
  int ret = OB_SUCCESS;
  TBSYS_LOG(INFO, "start master nameserver");

  if (OB_SUCCESS == ret) {
    request_cs_report_tablet();
    // after restart master name server refresh the schema
...
void NameServer::start_merge_check() {
...
int NameServer::renew_core_schema(void) {
  int ret = OB_ERROR;
...
int NameServer::renew_user_schema(int64_t& count) {
  int ret = OB_ERROR;
  const static int64_t retry_times = 3;
  for (int64_t i = 0; (ret != OB_SUCCESS) && (i < retry_times); ++i) {
...
int NameServer::boot_strap(void) {
  int ret = OB_ERROR;
  TBSYS_LOG(INFO, "NameServer::bootstrap() start");
  ObBootstrap bootstrap(*this);
  ...
  TBSYS_LOG(INFO, "NameServer::bootstrap() end");
  return ret;
}

int NameServer::do_bootstrap(ObBootstrap& bootstrap) {
  int ret = OB_SUCCESS;
...
int NameServer::after_boot_strap(ObBootstrap& bootstrap) {
  int ret = OB_SUCCESS;
  ...
  if ((OB_SUCCESS == ret) && (role.get_role() == ObiRole::MASTER)) {
    ret = NameServerTriggerUtil::slave_boot_strap(root_trigger_);
    if (ret != OB_SUCCESS) {
...
int NameServer::slave_boot_strap() {
  int ret = OB_SUCCESS;
  TBSYS_LOG(INFO, "NameServer::bootstrap() start");
  ObBootstrap bootstrap(*this);
  if (!boot_state_.can_boot_strap()) {
...
int NameServer::after_restart() {
  ...
  if (OB_SUCCESS != ret) {
    ...
    //fix bug: update nameserver info to __all_server
    ObServer rs;
    config_.get_name_server(rs);
...
int NameServer::start_notify_switch_schema() {
  int ret = OB_SUCCESS;
  ObServer master_rs;
  ObServer slave_rs[OB_MAX_CLUSTER_COUNT];
  int64_t slave_count = OB_MAX_CLUSTER_COUNT;
  NameServerRpcStub& rpc_stub = get_rpc_stub();
  ObServer ms_server;
  master_rs.set_ipv4_addr(config_.master_name_server_ip, (int32_t)config_.master_name_server_port);
...
int NameServer::notify_switch_schema(bool only_core_tables, bool force_update) {
  int ret = OB_SUCCESS;
...
ObBootState* NameServer::get_boot() {
  return &boot_state_;
}
ObBootState::State NameServer::get_boot_state()const {
  return boot_state_.get_boot_state();
}

NameServerRpcStub& NameServer::get_rpc_stub() {
  return worker_->get_rpc_stub();
}

ObSchemaManagerV2* NameServer::get_ini_schema() const {
  return local_schema_manager_;
}

int NameServer::get_schema(const bool force_update, bool only_core_tables, ObSchemaManagerV2& out_schema) {
  int ret = OB_SUCCESS;
...
int64_t NameServer::get_last_frozen_version() const {
  return last_frozen_mem_version_;
}

int NameServer::switch_schema_manager(const ObSchemaManagerV2& schema_manager) {
  int ret = OB_SUCCESS;
...
int NameServer::get_table_id_name(ObTableIdNameIterator* table_id_name, bool& only_core_tables) {
  int ret = OB_SUCCESS;
  NameServerMsProvider ms_provider(server_manager_);
  ms_provider.init(config_, worker_->get_rpc_stub());
  ObUps ups_master;
  ups_manager_->get_ups_master(ups_master);
  NameServerUpsProvider ups_provider(ups_master.addr_);
...
int NameServer::get_table_schema(const uint64_t table_id, TableSchema& table_schema) {
  ObString table_name;
...
int64_t NameServer::get_privilege_version() const {
  return privilege_timestamp_;
}

void NameServer::set_privilege_version(const int64_t privilege_version) {
  privilege_timestamp_ = privilege_version;
}

void NameServer::set_daily_merge_tablet_error(const char* err_msg, const int64_t length) {
  state_.set_daily_merge_error(err_msg, length);
}

void NameServer::clean_daily_merge_tablet_error() {
  state_.clean_daily_merge_error();
}

bool NameServer::is_daily_merge_tablet_error()const {
  return state_.is_daily_merge_tablet_error();
}
char* NameServer::get_daily_merge_error_msg() {
  return state_.get_error_msg();
}
int64_t NameServer::get_config_version() const {
  return worker_->get_config_mgr().get_version();
}

int NameServer::get_max_tablet_version(int64_t& version) const {
  int err = OB_SUCCESS;
  version = -1;
  if (NULL != root_table_) {
...
bool NameServer::check_root_table(const common::ObServer& expect_cs) const {
  bool err = false;
  tbsys::CRLockGuard guard(root_table_rwlock_);
...
int NameServer::dump_cs_tablet_info(const common::ObServer& cs, int64_t& tablet_num) const {
  int err = OB_SUCCESS;
  tbsys::CRLockGuard guard(root_table_rwlock_);
...
int NameServer::try_create_new_table(int64_t frozen_version, const uint64_t table_id) {
  int err = OB_SUCCESS;
...
int NameServer::create_table_tablets(const uint64_t table_id, const ObTabletInfoList& list) {
  int err = OB_SUCCESS;
  int64_t version = -1;
...
int64_t NameServer::get_alive_cs_number() {
  int64_t number = 0;
  ObChunkServerManager::iterator it = server_manager_.begin();
  for (; it != server_manager_.end(); ++it) {
    if (it->port_cs_ != 0
...
int NameServer::check_tablets_legality(const common::ObTabletInfoList& tablets) {
  int err = OB_SUCCESS;
...
int NameServer::try_create_new_tables(int64_t frozen_version) {
  int ret = OB_SUCCESS;
  int err = OB_SUCCESS;
...
int NameServer::create_tablet_with_range(const int64_t frozen_version, const ObTabletInfoList& tablets) {
  int ret = OB_SUCCESS;
...
int NameServer::check_tablet_version(const int64_t tablet_version, const int64_t safe_count, bool& is_merged) const {
  int err = OB_SUCCESS;
 ...
 */
int NameServer::regist_chunk_server(const ObServer& server, const char* server_version, int32_t& status, int64_t time_stamp) {
  int ret = OB_SUCCESS;
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (master) {
      if (!is_listen_ms) {
        ...
        config_.get_name_server(cluster_vip);
...
int NameServer::migrate_over(const int32_t result, const ObDataSourceDesc& desc,
  ...
  if (!is_loading_data() && state_.get_bypass_flag()) {
    TBSYS_LOG(WARN, "nameserver in clean root table process, refuse to migrate over report");
...
int NameServer::find_monitor_table_key(const common::ObGetParam& get_param, common::ObScanner& scanner) {
  int ret = OB_SUCCESS;
  ...
  if (NULL == (cell = get_param[0])) {
    ...
    NameServerMonitorTable monitor_table;
...
int NameServer::find_session_table_key(const common::ObGetParam& get_param, common::ObScanner& scanner) {
  int ret = OB_SUCCESS;
  const ObCellInfo* cell = NULL;
...
int NameServer::find_statement_table_key(const common::ObGetParam& get_param, common::ObScanner& scanner) {
  int ret = OB_SUCCESS;
  const ObCellInfo* cell = NULL;
...
int NameServer::find_root_table_key(const common::ObGetParam& get_param, common::ObScanner& scanner) {
  int ret = OB_SUCCESS;
  const ObCellInfo* cell = NULL;
...
int NameServer::get_rowkey_info(const uint64_t table_id, ObRowkeyInfo& info) const {
  int ret = OB_SUCCESS;
  // @todo
  UNUSED(table_id);
  UNUSED(info);
...
int NameServer::find_root_table_range(const common::ObScanParam& scan_param, ObScanner& scanner) {
 ...
 *
 */
int NameServer::waiting_job_done(const common::ObServer& server, const int64_t frozen_mem_version) {
...
int NameServer::got_reported(const ObTabletReportInfoList& tablets, const int server_index,
  ...
  if (ObBootState::OB_BOOT_OK != boot_state_.get_boot_state() && !is_replay_log) {
    ...
    if (0 < delete_list_.get_tablet_size()) {
      if (is_master() || worker_->get_role_manager()->get_role() == ObRoleMgr::STANDALONE) {
        //NameServerUtil::delete_tablets(worker_->get_rpc_stub(), server_manager_, delete_list_, config_.network_timeout);
...
int64_t NameServer::get_table_count(void) const {
  int64_t ret = 0;
  tbsys::CRLockGuard guard(schema_manager_rwlock_);
  if (NULL == schema_manager_for_cache_) {
...
void NameServer::get_tablet_info(int64_t& tablet_count, int64_t& row_count, int64_t& data_size) const {
  tbsys::CRLockGuard guard(root_table_rwlock_);
  if (NULL != root_table_) {
...
int NameServer::get_row_checksum(const int64_t tablet_version, const uint64_t table_id, ObRowChecksum& row_checksum) {
  int ret = OB_SUCCESS;
...
// just for replay name server remove rplica commit log
...
int NameServer::delete_tables(const bool did_replay, const common::ObArray<uint64_t>& deleted_tables) {
...
int NameServer::select_cs(const int64_t min_count, ObArray<std::pair<ObServer, int32_t> >& chunkservers) {
  int ret = OB_SUCCESS;
...
int NameServer::fetch_mem_version(int64_t& mem_version) {
  int ret = OB_SUCCESS;
  ObUps ups_master;
  if (OB_SUCCESS != (ret = ups_manager_->get_ups_master(ups_master))) {
...
int NameServer::create_empty_tablet(TableSchema& tschema, ObArray<ObServer>& created_cs) {
  int ret = OB_SUCCESS;
  if (!is_master()) {
    ret = OB_NOT_MASTER;
...
int NameServer::delete_dropped_tables(int64_t& table_count) {
  OB_ASSERT(balancer_);
  int ret = OB_SUCCESS;
  uint64_t table_id = OB_INVALID_ID;
  common::ObArray<uint64_t> deleted_tables;
...
int NameServer::check_table_exist(const common::ObString& table_name, bool& exist) {
  int ret = OB_SUCCESS;
  tbsys::CRLockGuard guard(schema_manager_rwlock_);
  if (NULL == schema_manager_for_cache_) {
    TBSYS_LOG(WARN, "check schema manager failed");
...
int NameServer::switch_ini_schema() {
  int ret = OB_SUCCESS;
  tbsys::CConfig config;
  if (!local_schema_manager_->parse_from_file(config_.schema_filename, config)) {
    ...
    if (ret != OB_SUCCESS) {
      ...
      ret = NameServerTriggerUtil::create_table(root_trigger_);
      if (ret != OB_SUCCESS) {
...
int NameServer::refresh_new_schema(int64_t& table_count) {
  int ret = OB_SUCCESS;
...
/// for sql api
int NameServer::alter_table(common::AlterTableSchema& tschema) {
  ...
  if (ret != OB_SUCCESS) {
    ...
    // fire an event to tell all clusters
    {
      err = NameServerTriggerUtil::alter_table(root_trigger_);
      if (err != OB_SUCCESS) {
...
/// for sql api
int NameServer::create_table(bool if_not_exists, const common::TableSchema& tschema) {
  int ret = OB_SUCCESS;
  ...
  if (OB_SUCCESS == ret) {
    ...
    // fire an event to tell all clusters
    {
      err = NameServerTriggerUtil::create_table(root_trigger_, tschema.table_id_);
      if (err != OB_SUCCESS) {
...
/// for sql api
int NameServer::drop_tables(bool if_exists, const common::ObStrings& tables) {
  ...
  if (ret != OB_SUCCESS) {
    ...
    // fire an event to tell all clusters
    {
      int err = NameServerTriggerUtil::notify_slave_refresh_schema(root_trigger_);
      if (err != OB_SUCCESS) {
...
int NameServer::drop_one_table(const bool if_exists, const ObString& table_name, bool& refresh) {
  ...
  if ((OB_SUCCESS == ret) && exist) {
    ret = NameServerTriggerUtil::drop_tables(root_trigger_, table_id);
    if (ret != OB_SUCCESS) {
...
int NameServer::get_master_ups(ObServer& ups_addr, bool use_inner_port) {
  int ret = OB_ENTRY_NOT_EXIST;
  ObUps ups_master;
  if (NULL != ups_manager_) {
    if (OB_SUCCESS != (ret = ups_manager_->get_ups_master(ups_master))) {
...
ObServer NameServer::get_update_server_info(bool use_inner_port) const {
  ObServer server;
  ObUps ups_master;
  if (NULL != ups_manager_) {
    ups_manager_->get_ups_master(ups_master);
...
int NameServer::get_table_info(const common::ObString& table_name, uint64_t& table_id, int32_t& max_row_key_length) {
  int err = OB_SUCCESS;
...
bool NameServer::check_all_tablet_safe_merged(void) const {
  bool ret = false;
...
int NameServer::report_frozen_memtable(const int64_t frozen_version, const int64_t last_frozen_time, bool did_replay) {
  int ret = OB_SUCCESS;
...
int NameServer::get_last_frozen_version_from_ups(const bool did_replay) {
...
int NameServer::request_cs_report_tablet() {
  int ret = OB_SUCCESS;
...
int NameServer::get_master_ups_config(int32_t& master_master_ups_read_percent, int32_t& slave_master_ups_read_percent) const {
  int ret = OB_SUCCESS;
...
void NameServer::cancel_restart_all_cs() {
  tbsys::CWLockGuard guard(server_manager_rwlock_);
  server_manager_.cancel_restart_all_cs();
}

void NameServer::restart_all_cs() {
  tbsys::CWLockGuard guard(server_manager_rwlock_);
...
int NameServer::shutdown_cs(const common::ObArray<common::ObServer>& servers, enum ShutdownOperation op) {
  int ret = OB_SUCCESS;
  tbsys::CWLockGuard guard(server_manager_rwlock_);
...
int NameServer::cancel_shutdown_cs(const common::ObArray<common::ObServer>& servers, enum ShutdownOperation op) {
  int ret = OB_SUCCESS;
  tbsys::CWLockGuard guard(server_manager_rwlock_);
...
int NameServer::table_exist_in_cs(const uint64_t table_id, bool& is_exist) {
  int ret = OB_SUCCESS;
  is_exist = false;
  ObChunkServerManager::iterator it = server_manager_.begin();
  for (; it != server_manager_.end(); ++it) {
...
void NameServer::do_stat_cs_num(char* buf, const int64_t buf_len, int64_t& pos) {
  int num = 0;
  ObChunkServerManager::iterator it = server_manager_.begin();
  for (; it != server_manager_.end(); ++it) {
    if (it->port_cs_ != 0
...
void NameServer::do_stat_ms_num(char* buf, const int64_t buf_len, int64_t& pos) {
  int num = 0;
  ObChunkServerManager::iterator it = server_manager_.begin();
  for (; it != server_manager_.end(); ++it) {
    if (it->port_ms_ != 0
...
void NameServer::do_stat_all_server(char* buf, const int64_t buf_len, int64_t& pos) {
  if (NULL != ups_manager_) {
...
int64_t NameServer::get_stat_value(const int32_t index) {
  int64_t ret = 0;
  ObStat* stat = NULL;
...
void NameServer::do_stat_merge(char* buf, const int64_t buf_len, int64_t& pos) {
...
void NameServer::do_stat_unusual_tablets_num(char* buf, const int64_t buf_len, int64_t& pos) {
  int32_t num = 0;
  tbsys::CRLockGuard guard(root_table_rwlock_);
  if (root_table_ != NULL) {
    root_table_->dump_unusual_tablets(last_frozen_mem_version_,
...
int NameServer::do_stat(int stat_key, char* buf, const int64_t buf_len, int64_t& pos) {
  ...
  case OB_RS_STAT_RS_STATUS:
    databuff_printf(buf, buf_len, pos, "ns_status: INITED");
    break;
...
int NameServer::register_ups(const common::ObServer& addr, int32_t inner_port, int64_t log_seq_num, int64_t lease, const char* server_version) {
  int ret = OB_SUCCESS;
  if (NULL == ups_manager_) {
...
int NameServer::ups_slave_failure(const common::ObServer& addr, const common::ObServer& slave_addr) {
  int ret = OB_SUCCESS;
  if (NULL == ups_manager_) {
    TBSYS_LOG(ERROR, "ups_manager is NULL");
    ret = OB_NOT_INIT;
...
int NameServer::set_ups_config(int32_t read_master_master_ups_percentage, int32_t read_slave_master_ups_percentage) {
  int ret = OB_SUCCESS;
  if (NULL == ups_manager_) {
    TBSYS_LOG(ERROR, "ups_manager is NULL");
    ret = OB_NOT_INIT;
...
int NameServer::change_ups_master(const common::ObServer& ups, bool did_force) {
  int ret = OB_SUCCESS;
  if (NULL == ups_manager_) {
    TBSYS_LOG(ERROR, "ups_manager is NULL");
    ret = OB_NOT_INIT;
...
int NameServer::get_ups_list(common::ObUpsList& ups_list) {
  int ret = OB_SUCCESS;
  if (NULL == ups_manager_) {
    TBSYS_LOG(ERROR, "ups_manager is NULL");
    ret = OB_NOT_INIT;
...
int NameServer::serialize_proxy_list(char* buf, const int64_t buf_len, int64_t& pos) const {
  int ret = OB_SUCCESS;
...
int NameServer::grant_eternal_ups_lease() {
  int ret = OB_SUCCESS;
  if (NULL == ups_heartbeat_thread_
...
int NameServer::cs_import_tablets(const uint64_t table_id, const int64_t tablet_version) {
  int ret = OB_SUCCESS;
  if (OB_INVALID_ID == table_id) {
...
int NameServer::receive_hb(const common::ObServer& server, const int32_t sql_port, const bool is_listen_ms, const ObRole role) {
  int64_t  now = tbsys::CTimeUtil::getTime();
...
int NameServer::force_heartbeat_all_servers(void) {
...
int NameServer::force_sync_schema_all_servers(const ObSchemaManagerV2& schema) {
  int ret = OB_SUCCESS;
  ObServer tmp_server;
  if (this->is_master()) {
    ObChunkServerManager::iterator it = this->server_manager_.begin();
...
//for bypass
int NameServer::prepare_bypass_process(common::ObBypassTaskInfo& table_name_id) {
  int ret = OB_SUCCESS;
...
int NameServer::write_schema_to_file() {
  int ret = OB_SUCCESS;
  ObSchemaManagerV2* out_schema = OB_NEW(ObSchemaManagerV2, ObModIds::OB_RS_SCHEMA_MANAGER);
  if (NULL == out_schema) {
    ret = OB_MEM_OVERFLOW;
...
int NameServer:: get_new_table_id(uint64_t& max_table_id, ObBypassTaskInfo& table_name_id) {
  int ret = OB_SUCCESS;
...
int NameServer::slave_clean_root_table() {
  int ret = OB_SUCCESS;

  {
    ...
    tbsys::CThreadGuard mutex_gard(&root_table_build_mutex_);
    tbsys::CWLockGuard guard(root_table_rwlock_);
    TBSYS_LOG(INFO, "slave nameserver start to clean root table, and set bypass version to zero");
    root_table_->clear();
  }
...
int NameServer::clean_root_table() {
  int ret = OB_SUCCESS;
  ...
  if (OB_SUCCESS != ret) {
    ...
    //bugfix
    //send clean_root_table status to slave nameserver
...
int NameServer::start_bypass_process(ObBypassTaskInfo& table_name_id) {
  int ret = OB_SUCCESS;
  bool is_all_merged = false;
...
int NameServer::check_bypass_process() {
  int ret = OB_SUCCESS;
  if (!state_.get_bypass_flag()) {
    TBSYS_LOG(WARN, "nameserver have no bypass duty now.");
    ret = OB_ERROR;
  }
  OperationType type;
  if (OB_SUCCESS == ret) {
...
int NameServer::switch_bypass_schema(common::ObSchemaManagerV2* schema_manager, common::ObArray<uint64_t>& delete_tables) {
  int ret = OB_SUCCESS;
  delete_tables.clear();
  if (NULL == schema_manager) {
...
bool NameServer::is_bypass_process() {
...
int NameServer::cs_delete_table_done(const ObServer& cs,
  ...
  if (is_loading_data()) {
    ...
    ret = OB_ERROR;
    TBSYS_LOG(WARN, "nameserver not in bypass process.");
  }
  return ret;
}

void NameServer::set_bypass_version(const int64_t version) {
  // should only used in log worker during log replay
  tbsys::CThreadGuard mutex_guard(&frozen_version_mutex_);
  bypass_process_frozen_men_version_ = version;
}

NameServerOperationHelper* NameServer::get_bypass_operation() {
  return &operation_helper_;
}

int NameServer::unlock_frozen_version() {
  int ret = OB_SUCCESS;
  tbsys::CThreadGuard mutex_guard(&frozen_version_mutex_);
...
int NameServer::lock_frozen_version() {
  int ret = OB_SUCCESS;
  tbsys::CThreadGuard mutex_guard(&frozen_version_mutex_);
  if (bypass_process_frozen_men_version_ > 0) {
    ret = OB_EAGAIN;
...
int64_t NameServer::get_frozen_version_for_cs_heartbeat() const {
  tbsys::CThreadGuard mutex_guard(&frozen_version_mutex_);
  int64_t ret = last_frozen_mem_version_;
  if (bypass_process_frozen_men_version_ > 0) {
    TBSYS_LOG(DEBUG, "nameserver in bypass process. broadcast version=%ld", bypass_process_frozen_men_version_);
...
int NameServer::change_table_id(const int64_t table_id, const int64_t new_table_id) {
  int ret = OB_SUCCESS;
  {
...
int NameServer::add_range_for_load_data(const common::ObList<ObNewRange*>& range_table) {
...
int NameServer::add_range_to_root_table(const ObTabletReportInfoList& tablets, const bool is_replay_log) {
  int ret = OB_SUCCESS;
  ObServer server;
...
int NameServer::change_table_id(const ObString& table_name, const uint64_t new_table_id) {
  int ret = OB_SUCCESS;
...
int NameServer::get_table_id(const ObString table_name, uint64_t& table_id) {
  int ret = OB_SUCCESS;
  TableSchema table_schema;
  const bool only_core_tables = false;
  schema_service_->init(schema_service_scan_helper_, only_core_tables);
...
int NameServer::load_data_done(const ObString table_name, const uint64_t old_table_id) {
  int ret = OB_SUCCESS;
  TableSchema table_schema;
  const bool only_core_tables = false;
  schema_service_->init(schema_service_scan_helper_, only_core_tables);
...
int NameServer::load_data_fail(const uint64_t new_table_id) {
  int ret = OB_SUCCESS;
  if (!is_master()) {
    ret = OB_RS_STATE_NOT_ALLOW;
    TBSYS_LOG(ERROR, "only the master rs can do load_data_done");
...
int NameServer::init_first_meta() {
  int ret = OB_SUCCESS;
...
int NameServer::start_import(const ObString& table_name, const uint64_t table_id, ObString& uri) {
  int ret = OB_SUCCESS;
  ObServer master_rs;
  ObServer slave_rs[OB_MAX_CLUSTER_COUNT];
  int64_t slave_count = OB_MAX_CLUSTER_COUNT;
  ...
  ObServer ms_server;
  master_rs.set_ipv4_addr(config_.master_name_server_ip, static_cast<int32_t>(config_.master_name_server_port));
...
int NameServer::import(const ObString& table_name, const uint64_t table_id, ObString& uri, const int64_t start_time) {
  int ret = OB_SUCCESS;
...
int NameServer::start_kill_import(const ObString& table_name, const uint64_t table_id) {
  int ret = OB_SUCCESS;
  ObServer master_rs;
  ObServer slave_rs[OB_MAX_CLUSTER_COUNT];
  int64_t slave_count = OB_MAX_CLUSTER_COUNT;
...
int NameServer::kill_import(const ObString& table_name, const uint64_t table_id) {
  int ret = OB_SUCCESS;
...
int NameServer::set_bypass_flag(const bool flag) {
  return state_.set_bypass_flag(flag);
}

bool NameServer::is_loading_data() const {
  OB_ASSERT(balancer_);
  return balancer_->is_loading_data();
}

int NameServer::trigger_create_table(const uint64_t table_id/* =0 */) {
  int ret = OB_SUCCESS;
  ObSchemaManagerV2* out_schema = OB_NEW(ObSchemaManagerV2, ObModIds::OB_RS_SCHEMA_MANAGER);
...
int NameServer::trigger_drop_table(const uint64_t table_id) {
  int ret = OB_SUCCESS;
...
int NameServer::force_drop_table(const uint64_t table_id) {
  int ret = OB_SUCCESS;
  int64_t table_count = 0;
...
int NameServer::force_create_table(const uint64_t table_id) {
  int ret = OB_SUCCESS;
...
int NameServer::get_ms(ObServer& ms_server) {
  int ret = OB_SUCCESS;
