...
NameServerBalancer::NameServerBalancer()
  : config_(NULL),
    root_table_(NULL),
...
void NameServerBalancer::check_components() const {
  OB_ASSERT(config_);
  OB_ASSERT(rpc_stub_);
...
void NameServerBalancer::do_balance_or_load() {
  check_components();
...
inline int NameServerBalancer::need_copy(int32_t available_num, int32_t lost_num) {
...
int32_t NameServerBalancer::nb_get_table_count() {
  ...
  if (NULL == schema_manager) {
    ...
    ret = name_server_->get_schema(false, false, *schema_manager);
    if (OB_SUCCESS != ret) {
...
uint64_t NameServerBalancer::nb_get_next_table_id(int32_t table_count, int32_t seq/*=-1*/) {
  ...
  if (0 >= table_count) {
    ...
    if (OB_SUCCESS == err) {
      err = name_server_->get_schema(false, false, *schema_manager);
      if (OB_SUCCESS != err) {
    ...
    if (OB_SUCCESS != (ret = delete_list_.add_tablet(to_delete))) {
      ...
      if (role_mgr_->is_master()) {
        if (OB_SUCCESS != log_worker_->remove_replica(to_delete)) {
          ...
          int tmp_ret = NameServerUtil::delete_tablets(*rpc_stub_, *server_manager_, delete_list_, config_->network_timeout);
...
bool NameServerBalancer::check_not_ini_table(const uint64_t table_id) const {
  bool exist = false;
  ObSchemaManagerV2* schema_manager = name_server_->get_ini_schema();
  if (NULL == schema_manager) {
    ...
    if (!table_found) {
      if (table_id < OB_APP_MIN_TABLE_ID) {
        ...
        TBSYS_LOG(ERROR, "find table not in root table:role[%d], master[%d], table_id[%lu]",
                  name_server_->get_obi_role().get_role(), name_server_->is_master(), table_id);
      }
    } else {
...
void NameServerBalancer::check_shutdown_process() {
...
int NameServerBalancer::nb_balance_by_table(const uint64_t table_id, bool& scan_next_table) {
  int ret = OB_SUCCESS;
  int64_t avg_size = 0;
  int64_t avg_count = 0;
  int32_t cs_num = 0;
...
bool NameServerBalancer::nb_is_curr_table_balanced(int64_t avg_sstable_count, const ObServer& except_cs) const {
  bool ret = true;
  int64_t delta_count = config_->balance_tolerance_count;
  int32_t cs_out = 0;
  int32_t cs_in = 0;
...
bool NameServerBalancer::nb_is_curr_table_balanced(int64_t avg_sstable_count) const {
...
void NameServerBalancer::nb_print_balance_info() const {
...
void NameServerBalancer::nb_print_balance_info(char* buf, const int64_t buf_len, int64_t& pos) const {
  char addr_buf[OB_IP_STR_BUFF];
  const ObServerStatus* it = server_manager_->begin();
  for (; it != server_manager_->end(); ++it) {
    if (NULL != it && ObServerStatus::STATUS_DEAD != it->status_) {
...
int NameServerBalancer::send_msg_migrate(const ObServer& dest, const ObDataSourceDesc& data_source_desc) {
  int ret = OB_SUCCESS;
...
void NameServerBalancer::nb_print_migrate_infos() const {
...
void NameServerBalancer::dump_migrate_info() const {
...
int NameServerBalancer::nb_trigger_next_migrate(const ObDataSourceDesc& desc, int32_t result) {
  check_components();
  int ret = OB_SUCCESS;
...
// 负载均衡入口函数
void NameServerBalancer::do_balance() {
  check_components();
...
int64_t NameServerBalancer::get_loading_data_infos_count() {
  int count = 0;
  for (int64_t i = 0; i < MAX_LOAD_INFO_CONCURRENCY; ++i) {
...
void NameServerBalancer::do_load_data() {
  check_components();
  int ret = OB_SUCCESS;
  int tmp_ret = OB_SUCCESS;
  if (OB_SUCCESS != (tmp_ret = restart_server_->restart_servers())) {
  ...
  if (!name_server_->is_master()) {
    ...
    while (scan_next_table) {
      ...
      } else if (!need_replicate && found_table && name_server_->get_obi_role().get_role() == ObiRole::MASTER) {
...
// only for test
bool NameServerBalancer::nb_is_all_tables_balanced(const common::ObServer& except_cs) {
  bool ret = true;
  int32_t table_count = nb_get_table_count();
...
// only for test
bool NameServerBalancer::nb_is_all_tables_balanced() {
  ObServer not_exist_cs;
  return nb_is_all_tables_balanced(not_exist_cs);
}

void NameServerBalancer::nb_print_balance_infos(char* buf, const int64_t buf_len, int64_t& pos) {
  int32_t table_count = nb_get_table_count();
  for (int32_t i = 0; i < table_count; ++i) { // for each table
    uint64_t table_id = OB_INVALID_ID;
    if (OB_INVALID_ID != (table_id = nb_get_next_table_id(table_count, i))) {
...
bool NameServerBalancer::nb_did_cs_have_no_tablets(const common::ObServer& cs) const {
  bool ret = false;
  int32_t cs_index = -1;
  {
    tbsys::CRLockGuard guard(*server_manager_rwlock_);
...
void NameServerBalancer::nb_print_shutting_down_progress(char* buf, const int64_t buf_len, int64_t& pos) {
  int ret = OB_SUCCESS;
...
int NameServerBalancer::add_load_table(const ObString& table_name, const uint64_t table_id, ObString& uri, const int64_t start_time) {
  check_components();
  int ret = OB_SUCCESS;
  ...
  if (OB_SUCCESS == ret) {
    if (OB_SUCCESS != (ret = name_server_->add_range_for_load_data(range_table))) {
  ...
  if (OB_SUCCESS == ret) {
    ...
    if (OB_SUCCESS == ret) {
      if (NULL == info) {
        ...
        if (OB_SUCCESS == ret && name_server_->is_master()) {
        ...
        if (name_server_->is_master() && name_server_->get_obi_role().get_role() == ObiRole::MASTER) {
...
int NameServerBalancer::handle_load_table_done(const uint64_t table_id) {
  check_components();
  int ret = OB_SUCCESS;
  ObLoadDataInfo info;
  ...
  if (OB_SUCCESS == ret && is_finished) {
    if (OB_SUCCESS != (ret = name_server_->change_table_id(info.table_name_, info.table_id_))) {
  ...
  if (OB_SUCCESS == ret && is_finished) {
    int tmp_ret = name_server_->start_notify_switch_schema();
    if (OB_SUCCESS != tmp_ret) {
...
void NameServerBalancer::handle_load_table_failed(const uint64_t table_id, const bool need_write_handle_failed_log /*=true*/) {
  int ret = OB_SUCCESS;
  ...
  if (OB_SUCCESS == ret) {
    if (name_server_->is_master() && need_write_handle_failed_log) {
    ...
    TBSYS_LOG(WARN, "[import] load table failed, table_name=%.*s table_id=%lu",
              table_name.length(), table_name.ptr(), table_id);
    if (OB_SUCCESS != (ret = name_server_->load_data_fail(table_id))) {
...
int NameServerBalancer::kill_load_table(const ObString& table_name, const uint64_t table_id) {
  int ret = OB_SUCCESS;
  int64_t end_time = 0;
...
void NameServerBalancer::update_load_table_history(const ObLoadDataInfo& info) {
...
int NameServerBalancer::delete_load_table_from_log(const uint64_t table_id, const int32_t status, const int64_t end_time) {
  int ret = OB_SUCCESS;
  tbsys::CWLockGuard guard(load_data_lock_);
...
bool NameServerBalancer::is_table_loading(uint64_t table_id) const {
...
int NameServerBalancer::write_to_file(const char* filename) {
...
int NameServerBalancer::read_from_file(const char* filename) {
  int ret = OB_SUCCESS;
  common::FileUtils fu;
  char* data_buffer = NULL;
  int64_t size = 0;
    ...
    for (int64_t i = 0; i < MAX_LOAD_INFO_CONCURRENCY; ++i) {
      if (load_data_infos_[i].table_id_ == table_id &&
        ...
        if (load_data_infos_[i].status_ == ObLoadDataInfo::DOING
          ...
          if (name_server_->is_master()) {
            log_worker_->delete_load_table(table_id, static_cast<int32_t>(status), end_time);
            if (OB_SUCCESS != name_server_->load_data_done(table_name, load_data_infos_[i].old_table_id_)) {
          ...
          if (name_server_->is_master()) {
            log_worker_->delete_load_table(table_id, static_cast<int32_t>(status), end_time);
            if (OB_SUCCESS != name_server_->load_data_fail(table_id)) {
...
int NameServerBalancer::is_table_replicated(const uint64_t table_id, const int64_t tablet_version, bool& is_replicated) {
  OB_ASSERT(config_);
  int ret = OB_SUCCESS;
  int64_t tablet_replicas_num = config_->tablet_replicas_num;
  RootTable::const_iterator it;
...
int NameServerBalancer::get_ms(ObServer& ms_server) {
  check_components();
  int ret = name_server_->get_ms_provider().get_ms(ms_server);
...
int NameServerBalancer::init_load_data_env() {
  // load_data_lock_ must be locked by caller
  int ret = OB_SUCCESS;
  bool has_set_bypass_flag = false;
  if (OB_SUCCESS != name_server_->set_bypass_flag(true)) {
    // this flag will also be set by "build root table", to change name of this method in 0.5
    ret = OB_EAGAIN;
    TBSYS_LOG(WARN, "name server is building root table, can't start load table");
  } else {
    has_set_bypass_flag = true;
  }

  if (OB_SUCCESS == ret) {
    if (OB_SUCCESS != (ret = name_server_->lock_frozen_version())) {
      TBSYS_LOG(ERROR, "failed to lock frozen version");
    }
  }

  if (OB_SUCCESS == ret) {
    ret = check_replica_count_for_import(name_server_->get_frozen_version_for_cs_heartbeat());
    if (OB_SUCCESS != ret) {
...
int NameServerBalancer::add_load_table_task_info(const ObString& table_name, const uint64_t table_id,
  ...
  if (OB_SUCCESS == ret) {
    ret = name_server_->get_table_id(table_name, old_table_id);
    if (OB_SUCCESS != ret) {
  ...
  // find free pos and add info
  if (OB_SUCCESS == ret) {
    int64_t tablet_version = name_server_->get_frozen_version_for_cs_heartbeat();
    for (int64_t i = 0; i < MAX_LOAD_INFO_CONCURRENCY && OB_SUCCESS == ret; ++i) {
...
int NameServerBalancer::check_replica_count_for_import(int64_t tablet_version) {
  check_components();
  int ret = OB_SUCCESS;
