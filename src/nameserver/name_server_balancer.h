...
#include "name_server_balancer_runnable.h"
...
class NameServerBalancer {
 public:
  ...
  void set_rpc_stub(NameServerRpcStub* rpc_stub);
  ...
  void set_balancer_thread(NameServerBalancerRunnable* balancer_thread) {balancer_thread_ = balancer_thread;}
 ...
 private:
  ...
  // disallow copy
  NameServerBalancer(const NameServerBalancer& other);
  NameServerBalancer& operator=(const NameServerBalancer& other);
 private:
  // constants
  ...
  NameServerBalancerRunnable* balancer_thread_;
};

inline void NameServerBalancer::set_name_server(NameServer* name_server) {
  name_server_ = name_server;
}
inline void NameServerBalancer::set_tablet_manager(ObTabletInfoManager* tablet_manager) {
  tablet_manager_ = tablet_manager;
}
inline void NameServerBalancer::set_boot_state(ObBootState* boot_state) {
...
inline void NameServerBalancer::set_server_manager(ObChunkServerManager* server_manager) {
  server_manager_ = server_manager;
  data_source_mgr_.set_server_manager(server_manager);
}
inline void NameServerBalancer::set_server_manager_lock(tbsys::CRWLock* server_manager_rwlock) {
  server_manager_rwlock_ = server_manager_rwlock;
}
inline void NameServerBalancer::set_rpc_stub(NameServerRpcStub* rpc_stub) {
  rpc_stub_ = rpc_stub;
  data_source_mgr_.set_rpc_stub(rpc_stub);
}
inline void NameServerBalancer::set_log_worker(NameServerLogWorker* log_worker) {
  log_worker_ = log_worker;
}
inline void NameServerBalancer::set_role_mgr(common::ObRoleMgr* role_mgr) {
  role_mgr_ = role_mgr;
}
inline void NameServerBalancer::set_ups_manager(ObUpsManager* ups_manager) {
  ups_manager_ = ups_manager;
}
inline void NameServerBalancer::set_restart_server(ObRestartServer* restart_server) {
