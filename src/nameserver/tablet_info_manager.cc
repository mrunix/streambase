...
void ObTabletInfoManager::read_from_hex(FILE* stream) {
  static char start_key_buff[OB_MAX_ROW_KEY_LENGTH * 2];
  static char end_key_buff[OB_MAX_ROW_KEY_LENGTH * 2];
  static char sb_start_key_buff[OB_MAX_ROW_KEY_LENGTH];
  static char sb_end_key_buff[OB_MAX_ROW_KEY_LENGTH];
  ...
  for (int64_t i = 0; i < size; i++) {
    ...
    str_to_hex(start_key_buff, strlen(start_key_buff), sb_start_key_buff, OB_MAX_ROW_KEY_LENGTH);
    str_to_hex(end_key_buff, strlen(end_key_buff), sb_end_key_buff, OB_MAX_ROW_KEY_LENGTH);

    tablet_info.range_.start_key_.assign_ptr(sb_start_key_buff, strlen(start_key_buff) / 2);
    tablet_info.range_.end_key_.assign_ptr(sb_end_key_buff, strlen(end_key_buff) / 2);
