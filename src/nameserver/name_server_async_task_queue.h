...
class NameServerAsyncTaskQueue {
 public:
  NameServerAsyncTaskQueue();
  virtual ~NameServerAsyncTaskQueue();
 public:
  struct ObSeqTask {
    ObTaskType type_;
    int32_t inner_port_;
    ...
    common::ObServer server_;  //server ip will using nameserver ip when lms registed
   ...
   protected:
    ...
    bool operator != (const ObSeqTask& other) const;
   private:
    friend class NameServerAsyncTaskQueue;
    friend class NameServerInnerTableTask;
    uint64_t task_id_;
    int64_t timestamp_;
  };
 public:
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(NameServerAsyncTaskQueue);
