#include "name_server_log_replay.h"

namespace sb {
namespace nameserver {
NameServerLogReplay::NameServerLogReplay() {
...
void NameServerLogReplay::set_log_manager(NameServerLogManager* log_manager) {
  log_manager_ = log_manager;
}

void NameServerLogReplay::wait_replay(ObLogCursor& end_cursor) {
...
int NameServerLogReplay::replay(common::LogCommand cmd, uint64_t seq, const char* log_data, const int64_t data_len) {
...
void NameServerLogReplay::run(tbsys::CThread* thread, void* arg) {
  int ret = OB_SUCCESS;
