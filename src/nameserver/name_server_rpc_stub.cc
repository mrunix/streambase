...
#include "nameserver/name_server_rpc_stub.h"
...
int NameServerRpcStub::init(const common::ObClientManager* client_mgr, common::ThreadSpecificBuffer* tsbuffer) {
...
int NameServerRpcStub::slave_register(const common::ObServer& master, const common::ObServer& slave_addr,
                                      ObFetchParam& fetch_param, const int64_t timeout) {
  int err = OB_SUCCESS;
  common::ObDataBuffer data_buff;
  ...
  int64_t pos = 0;
  if (OB_SUCCESS == err) {
    common::ObResultCode result_code;
...
int NameServerRpcStub::get_thread_buffer(common::ObDataBuffer& data_buffer) {
  int ret = OB_SUCCESS;
...
int NameServerRpcStub::get_row_checksum(const common::ObServer& server, const int64_t data_version, const uint64_t table_id, ObRowChecksum& row_checksum, int64_t timeout_us) {
  int ret = OB_SUCCESS;
  common::ObDataBuffer msgbuf;
...
int NameServerRpcStub::migrate_tablet(const common::ObServer& dest_server, const common::ObDataSourceDesc& desc, const int64_t timeout_us) {
...
int NameServerRpcStub::create_tablet(const common::ObServer& cs, const common::ObNewRange& range,
  ...
  int ret = OB_SUCCESS;
  common::ObDataBuffer msgbuf;
  static char buff[common::OB_MAX_PACKET_LENGTH];
  msgbuf.set_data(buff, common::OB_MAX_PACKET_LENGTH);
  if (NULL == client_mgr_) {
    TBSYS_LOG(ERROR, "client_mgr_=NULL");
    ret = OB_ERROR;
...
int NameServerRpcStub::delete_tablets(const common::ObServer& cs, const common::ObTabletReportInfoList& tablets, const int64_t timeout_us) {
  int ret = OB_SUCCESS;
  common::ObDataBuffer msgbuf;
...
int NameServerRpcStub::import_tablets(const common::ObServer& cs, const uint64_t table_id, const int64_t version, const int64_t timeout_us) {
  int ret = OB_SUCCESS;
  common::ObDataBuffer msgbuf;
  if (NULL == client_mgr_) {
    TBSYS_LOG(ERROR, "client_mgr_=NULL");
    ...
    TBSYS_LOG(ERROR, "failed to serialize keey_src, err=%d", ret);
  } else if (OB_SUCCESS != (ret = client_mgr_->send_request(cs, common::OB_CS_IMPORT_TABLETS, DEFAULT_VERSION, timeout_us, msgbuf))) {
    TBSYS_LOG(WARN, "failed to send request, err=%d", ret);
  } else {
    common::ObResultCode result;
    int64_t pos = 0;
...
int NameServerRpcStub::get_last_frozen_version(const common::ObServer& ups, const int64_t timeout_us, int64_t& frozen_version) {
  ...
  if (NULL == client_mgr_) {
    ...
    TBSYS_LOG(ERROR, "failed to get thread buffer, err=%d", ret);
  } else if (OB_SUCCESS != (ret = client_mgr_->send_request(ups, common::OB_UPS_GET_LAST_FROZEN_VERSION, DEFAULT_VERSION, timeout_us, msgbuf))) {
    TBSYS_LOG(WARN, "failed to send request, err=%d", ret);
  } else {
    common::ObResultCode result;
    int64_t pos = 0;
...
int NameServerRpcStub::heartbeat_to_cs(const common::ObServer& cs, const int64_t lease_time, const int64_t frozen_mem_version,
  ...
  } else if (OB_SUCCESS != (ret = client_mgr_->post_request(cs, common::OB_REQUIRE_HEARTBEAT, MY_VERSION, msgbuf))) {
    TBSYS_LOG(WARN, "failed to send request, err=%d", ret);
  } else {
    // success
  }
...
int NameServerRpcStub::grant_lease_to_ups(const common::ObServer& ups, ObMsgUpsHeartbeat& msg) {
  int ret = OB_SUCCESS;
  common::ObDataBuffer msgbuf;
  ...
  } else if (OB_SUCCESS != (ret = client_mgr_->post_request(ups, common::OB_RS_UPS_HEARTBEAT, msg.MY_VERSION, msgbuf))) {
    TBSYS_LOG(WARN, "failed to send request, err=%d", ret);
  } else {
    // success
  }
...
int NameServerRpcStub::request_report_tablet(const common::ObServer& chunkserver) {
  int ret = OB_SUCCESS;
  common::ObDataBuffer msgbuf;
  if (NULL == client_mgr_) {
    TBSYS_LOG(ERROR, "client_mgr_=NULL");
  ...
  if (OB_SUCCESS == ret) {
    ret = client_mgr_->post_request(chunkserver, common::OB_RS_REQUEST_REPORT_TABLET, DEFAULT_VERSION, msgbuf);
    if (OB_SUCCESS != ret) {
...
int NameServerRpcStub::revoke_ups_lease(const common::ObServer& ups, const int64_t lease, const common::ObServer& master, const int64_t timeout_us) {
  int ret = OB_SUCCESS;
  common::ObDataBuffer msgbuf;
  ...
  if (NULL == client_mgr_) {
    ...
    TBSYS_LOG(ERROR, "failed to serialize, err=%d", ret);
  } else if (OB_SUCCESS != (ret = client_mgr_->send_request(ups, common::OB_RS_UPS_REVOKE_LEASE, msg.MY_VERSION, timeout_us, msgbuf))) {
    TBSYS_LOG(WARN, "failed to send request, err=%d", ret);
  } else {
    // success
    common::ObResultCode result;
...
int NameServerRpcStub::get_ups_max_log_seq(const common::ObServer& ups, uint64_t& max_log_seq, const int64_t timeout_us) {
  int ret = OB_SUCCESS;
  common::ObDataBuffer msgbuf;
  if (NULL == client_mgr_) {
    TBSYS_LOG(ERROR, "client_mgr_=NULL");
    ...
    TBSYS_LOG(ERROR, "failed to get thread buffer, err=%d", ret);
  } else if (OB_SUCCESS != (ret = client_mgr_->send_request(ups, common::OB_RS_GET_MAX_LOG_SEQ, DEFAULT_VERSION, timeout_us, msgbuf))) {
    TBSYS_LOG(WARN, "failed to send request, err=%d", ret);
  } else {
    // success
    common::ObResultCode result;
...
int NameServerRpcStub::shutdown_cs(const common::ObServer& cs, bool is_restart, const int64_t timeout_us) {
  int ret = OB_SUCCESS;
  common::ObDataBuffer msgbuf;
  if (NULL == client_mgr_) {
    TBSYS_LOG(ERROR, "client_mgr_=NULL");
  ...
  } else if (OB_SUCCESS != (ret = client_mgr_->send_request(cs, common::OB_STOP_SERVER, DEFAULT_VERSION, timeout_us, msgbuf))) {
    TBSYS_LOG(WARN, "failed to send request, err=%d", ret);
  } else {
    // success
    common::ObResultCode result;
...
int NameServerRpcStub::get_split_range(const common::ObServer& ups, const int64_t timeout_us,
                                       const uint64_t table_id, const int64_t forzen_version, common::ObTabletInfoList& tablets) {
  int ret = OB_SUCCESS;
  common::ObDataBuffer msgbuf;
  if (NULL == client_mgr_) {
    TBSYS_LOG(ERROR, "client_mgr_=NULL");
  ...
  if (OB_SUCCESS == ret) {
    if (OB_SUCCESS != (ret = common::serialization::encode_vi64(msgbuf.get_data(), msgbuf.get_capacity(),
  ...
  if (OB_SUCCESS == ret) {
    if (OB_SUCCESS != (ret = client_mgr_->send_request(ups, common::OB_RS_FETCH_SPLIT_RANGE, DEFAULT_VERSION, timeout_us, msgbuf))) {
      TBSYS_LOG(WARN, "failed to send request, err=%d", ret);
    }
  }
  common::ObResultCode result;
...
int NameServerRpcStub::table_exist_in_cs(const common::ObServer& cs, const int64_t timeout_us,
  ...
  if (OB_SUCCESS == ret) {
    if (OB_SUCCESS != (ret = client_mgr_->send_request(cs, common::OB_CS_CHECK_TABLET, DEFAULT_VERSION, timeout_us, msgbuf))) {
      TBSYS_LOG(WARN, "failed to send request, err=%d", ret);
    }
  }
  common::ObResultCode result;
  ...
  if (OB_SUCCESS == ret) {
    if (common::OB_CS_TABLET_NOT_EXIST == result.result_code_) {
      ret = OB_SUCCESS;
...
int NameServerRpcStub::request_cs_delete_table(const common::ObServer& chunkserver,
  ...
  if (OB_SUCCESS == ret) {
    ret = common::serialization::encode_vi64(msgbuf.get_data(), msgbuf.get_capacity(), msgbuf.get_position(), table_id);
    if (OB_SUCCESS != ret) {
      TBSYS_LOG(WARN, "fail to serialize table_name_id, ret=%d", ret);
    }
  }
  if (OB_SUCCESS == ret) {
    ret = client_mgr_->send_request(chunkserver, common::OB_CS_DELETE_TABLE, MY_VERSION, timeout_us, msgbuf);
    if (OB_SUCCESS != ret) {
      TBSYS_LOG(WARN, "fail to post request to chunkserver. err=%d, chunkserver_addr=%s", ret, chunkserver.to_cstring());
    }
  }
...
int NameServerRpcStub::fetch_ms_list(const common::ObServer& rs, common::ObArray<common::ObServer>& ms_list, const int64_t timeout) {
  int ret = OB_SUCCESS;
  int MY_VERSION = 1;
  common::ObDataBuffer data_buff;
  if (NULL == client_mgr_) {
  ...
  int32_t ms_num = 0;
  if (OB_SUCCESS == ret) {
    ret = common::serialization::decode_vi32(data_buff.get_data(), data_buff.get_capacity(), data_buff.get_position(), &ms_num);
    if (OB_SUCCESS != ret) {
...
int NameServerRpcStub::get_boot_state(const common::ObServer& master, const int64_t timeout_us, bool& boot_ok) {
  int ret = OB_SUCCESS;
  boot_ok = false;
  common::ObDataBuffer msgbuf;
  if (NULL == client_mgr_) {
    ...
    TBSYS_LOG(ERROR, "failed to get thread buffer, err=%d", ret);
  } else if (OB_SUCCESS != (ret = client_mgr_->send_request(master, common::OB_GET_BOOT_STATE, DEFAULT_VERSION, timeout_us, msgbuf))) {
    TBSYS_LOG(WARN, "failed to send request, err=%d", ret);
  } else {
    common::ObResultCode result;
    int64_t pos = 0;
...
int NameServerRpcStub::fetch_range_table(const common::ObServer& data_source, const ObString& table_name,
                                         common::ObList<ObNewRange*>& range_table , common::ModuleArena& allocator, int64_t timeout) {
  ObString uri;
...
int NameServerRpcStub::fetch_range_table(const ObServer& data_source, const ObString& table_name, const ObString& uri,
  ...
  common::ObResultCode result_code; // ths result code of the packet
  ...
  ObObj start_key_objs[common::OB_MAX_ROWKEY_COLUMN_NUMBER];
  ObObj end_key_objs[common::OB_MAX_ROWKEY_COLUMN_NUMBER];
  ...
  while (OB_SUCCESS == ret && has_more) {
    ...
    if (OB_SUCCESS != (ret = result_code.deserialize(data_buff.get_data(), data_buff.get_position(), pos))) {
      ...
      for (int64_t i = 0; i < count; ++i) {
        range.start_key_.assign(start_key_objs, common::OB_MAX_ROWKEY_COLUMN_NUMBER);
        range.end_key_.assign(end_key_objs, common::OB_MAX_ROWKEY_COLUMN_NUMBER);
        ...
        if (NULL == buffer) {
          ...
          if (OB_SUCCESS != (ret = common::deep_copy_range(allocator, range, *new_range))) {
            ...
            if (is_first) {
              ...
              if (!new_range->start_key_.is_min_row()) {
                ret = common::OB_ROOT_RANGE_NOT_CONTINUOUS;
              ...
              ret = common::OB_ROOT_RANGE_NOT_CONTINUOUS;
              TBSYS_LOG(WARN, "cur range[%s] and last range[%s] not continuous", common::to_cstring(*new_range), common::to_cstring(*last_range));
            }
      ...
      if (OB_SUCCESS == ret && !has_more) {
        if (NULL == last_range) {
          ...
          ret = common::OB_ROOT_RANGE_NOT_CONTINUOUS;
          TBSYS_LOG(WARN, "the end key of the last range[%s] is not max", common::to_cstring(*last_range));
        }
      }
    }
...
int NameServerRpcStub::fetch_slave_cluster_list(const common::ObServer& ms,
                                                const common::ObServer& master_rs,
                                                common::ObServer* slave_cluster_rs,
                                                int64_t& ns_count, int64_t timeout) {
  int ret = OB_SUCCESS;
  char sql_buf[common::OB_SQL_LENGTH];
  ObString select_stmt;
  common::ObDataBuffer msgbuf;
  common::ObResultCode rs;
  int64_t session_id;
  ...
  if (OB_SUCCESS == ret) {
    int n = snprintf(sql_buf, common::OB_SQL_LENGTH,
                     "select cluster_vip, nameserver_port from %s where cluster_role = 2;",
                     common::OB_ALL_CLUSTER);
    if (n < 0 || n >= common::OB_SQL_LENGTH) {
    ...
    if (OB_SUCCESS != (ret = get_thread_buffer(msgbuf))) {
      ...
      do {
        ...
        if (OB_SUCCESS !=
          ...
          if (OB_SUCCESS != (ret = fill_slave_cluster_list(scanner, master_rs, slave_cluster_rs, ns_count))) {
...
int NameServerRpcStub::fill_slave_cluster_list(ObNewScanner& scanner, const common::ObServer& master_rs,
                                               common::ObServer* slave_cluster_rs, int64_t& ns_count) {
  int ret = OB_SUCCESS;
  ObRow row;
...
int NameServerRpcStub::get_import_status(const common::ObServer& rs, const common::ObString& table_name,
  ...
  } else if (OB_SUCCESS != (ret = client_mgr_->send_request(rs, common::OB_RS_GET_IMPORT_STATUS, version, timeout_us, msgbuf))) {
    TBSYS_LOG(WARN, "failed to send request, err=%d", ret);
  } else {
    common::ObResultCode result;
    int64_t pos = 0;
...
int NameServerRpcStub::set_import_status(const common::ObServer& rs, const common::ObString& table_name,
  ...
  } else if (OB_SUCCESS != (ret = client_mgr_->send_request(rs, common::OB_RS_SET_IMPORT_STATUS, version, timeout_us, msgbuf))) {
    TBSYS_LOG(WARN, "failed to send request, err=%d", ret);
  } else {
    common::ObResultCode result;
    int64_t pos = 0;
...
int NameServerRpcStub::import(const common::ObServer& rs, const common::ObString& table_name,
  ...
  } else if (OB_SUCCESS != (ret = client_mgr_->send_request(rs, common::OB_RS_IMPORT, version, timeout_us, msgbuf))) {
    TBSYS_LOG(WARN, "failed to send request, err=%d", ret);
  } else {
    common::ObResultCode result;
    int64_t pos = 0;
...
int NameServerRpcStub::kill_import(const common::ObServer& rs, const common::ObString& table_name,
  ...
  if (NULL == client_mgr_) {
    ...
    TBSYS_LOG(ERROR, "failed to serialize table_id, err=%d", ret);
  } else if (OB_SUCCESS != (ret = client_mgr_->send_request(rs, common::OB_RS_KILL_IMPORT, version, timeout_us, msgbuf))) {
    TBSYS_LOG(WARN, "failed to send request, err=%d", ret);
  } else {
    common::ObResultCode result;
    int64_t pos = 0;
