...
int NameServerRpcStub::switch_schema(const common::ObServer& server, const common::ObSchemaManagerV2& schema_manager, const int64_t timeout_us) {
  int ret = OB_SUCCESS;
  ObDataBuffer msgbuf;
...
int NameServerRpcStub::execute_sql(const ObServer& ms, const ObString sql, int64_t timeout) {
  int ret = OB_SUCCESS;
...
int NameServerRpcStub::fetch_ms_list(const ObServer& rs, ObArray<ObServer>& ms_list, const int64_t timeout) {
  int ret = OB_SUCCESS;
  int MY_VERSION = 1;
  ObDataBuffer data_buff;
  if (NULL == client_mgr_) {
...
int NameServerRpcStub::fill_proxy_list(ObDataSourceProxyList& proxy_list, common::ObNewScanner& scanner) {
  int ret = OB_SUCCESS;
  ObRow row;
...
int NameServerRpcStub::fetch_slave_cluster_list(const common::ObServer& ms,
  ...
  if (OB_SUCCESS == ret) {
    int n = snprintf(sql_buf, OB_SQL_LENGTH, "select cluster_vip, nameserver_port from %s where cluster_role = 2;",
                     OB_ALL_CLUSTER);
    if (n < 0 || n >= OB_SQL_LENGTH) {
      ret = OB_BUF_NOT_ENOUGH;
...
int NameServerRpcStub::notify_switch_schema(const common::ObServer& rs, const int64_t timeout_us) {
  int ret = OB_SUCCESS;
  const int32_t version = 1;
  ObDataBuffer msgbuf;
  if (NULL == client_mgr_) {
