...
#include "nameserver/name_server_operation_helper.h"
namespace sb {
namespace nameserver {
NameServerOperationHelper::NameServerOperationHelper()
  : process_(INIT), name_server_(NULL),
...
int NameServerOperationHelper::init(const NameServer* name_server,
  ...
  if (NULL == name_server || NULL == config
    ...
    TBSYS_LOG(WARN, "invalid arugment, name_server=%p, config=%p, rpc_stub=%p, server_manager=%p",
              name_server, config, rpc_stub, server_manager);
...
int NameServerOperationHelper::clean_root_table(const common::ObSchemaManagerV2* schema_mgr) {
  int ret = OB_SUCCESS;
  ...
  if (OB_SUCCESS == ret) {
    ...
    if (OB_SUCCESS != ret) {
      ...
      TBSYS_LOG(INFO, "name server enter report_tablet process");
...
int NameServerOperationHelper::import_all_table(const int64_t frozen_version,
  ...
  if (OB_SUCCESS == ret) {
    ...
    if (OB_SUCCESS != (ret = request_cs_load_bypass_table(frozen_version, table_name_id))) {
      ...
      TBSYS_LOG(INFO, "name server enter load_sstable process");
...
int NameServerOperationHelper::check_delete_tables_process() {
  int ret = OB_ERROR;
...
int NameServerOperationHelper::waiting_job_done(const int cs_index) {
  int ret = OB_SUCCESS;
  ObServerStatus* status = server_manager_->get_server_status(cs_index);
  if (NULL == status) {
    ret = OB_ERROR;
...
int NameServerOperationHelper::end_bypass_process() {
  int ret = OB_SUCCESS;
  delete_index_ = 0;
  done_count_ = 0;
  total_count_ = 0;
...
int NameServerOperationHelper::set_delete_table(common::ObArray<uint64_t>& delete_tables) {
  int ret = OB_SUCCESS;
...
int NameServerOperationHelper::check_load_sstable_process() {
  int ret = OB_SUCCESS;
  ObChunkServerManager::iterator it = server_manager_->begin();
  for (; it != server_manager_->end(); ++it) {
    if (it->status_ != ObServerStatus::STATUS_DEAD
...
int NameServerOperationHelper::check_report_tablet_process() {
  int ret = OB_SUCCESS;
  ObChunkServerManager::iterator it = server_manager_->begin();
  for (; it != server_manager_->end(); ++it) {
    if (it->status_ != ObServerStatus::STATUS_DEAD
...
int NameServerOperationHelper::check_root_table() {
  int ret = OB_SUCCESS;
...
int NameServerOperationHelper::check_process(OperationType& type) {
  int ret = OB_SUCCESS;
  ...
  if (process_ == LOAD_SSTABLE) {
    ...
    if (OB_SUCCESS == ret) {
      ...
      process_ = REPORT_TABLET;
      start_time_us_ = tbsys::CTimeUtil::getTime();
      name_server_->request_cs_report_tablet();
  ...
  if (SORT_ROOT_TABLE == process_
    ...
    if (OB_SUCCESS == ret) {
      ...
      ret = name_server_->bypass_meta_data_finished(type, bypass_data_.get_root_table(),
...
NameServerOperationHelper::OperationProcess NameServerOperationHelper::get_process() {
  return process_;
}
void NameServerOperationHelper::set_process(OperationProcess process) {
  process_ = process;
}
const char* NameServerOperationHelper::print_process() {
  switch (process_) {
  case INIT:
    return "INIT";
  case STARTED:
