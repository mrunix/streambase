...
int ObLsyncServer:: get_log(ObFetchLogRequest& req, char* buf, int64_t limit, int64_t& pos, int64_t timeout) { //TODO:
  ...
  TBSYS_LOG(DEBUG, "get_log(log_file_id=%ld, log_seq_id=%ld, timeout=%ld)", log_file_id, log_seq_id, timeout);
  return reader_.get_log_entries_buf(log_file_id,  log_seq_id, buf, limit, pos, timeout - HANDLE_REQUEST_TIMEOUT_DELTA);
}

int ObLsyncServer::initialize(const char* log_dir, uint64_t log_start, const char* dev, int port, int64_t timeout) {
  int err = OB_SUCCESS;
  ...
  } else if (OB_SUCCESS != (err = reader_.initialize(log_dir))) {
    TBSYS_LOG(ERROR, "reader_.initialize(log_dir='%s', timeout=%ld)=>%d", log_dir, timeout, err);
  }
...
int ObLsyncServer::send_log_(ObFetchLogRequest& req, tbnet::Connection* conn, const uint32_t channel_id, int64_t timeout) {
  ...
  // shouldn't send response incase of timeout.
  if (OB_SUCCESS != ret) {
    return ret;
  }
...
int ObLsyncServer::send_response_packet(int __attribute__((unused))packet_code, int version, ObLsyncPacket* packet,
