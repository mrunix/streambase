...
#define getter_define(type, lower) inline type get_##lower() const { return lower##_; }
#define setter_define(type, lower) inline void set_##lower(type lower) { lower##_ = lower; }
...
class ObLsyncServerParam {
 ...
 public:
  config_item_define(const char*, dev_name, DEV_NAME);
  config_item_define(int, port, PORT);
  config_item_define(const char*, commit_log_dir, COMMIT_LOG_DIR);
