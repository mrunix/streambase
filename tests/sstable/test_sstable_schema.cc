 ...
 *
 * test_sstable_schema.cc for test sstable schema
 *
 * Authors:
...
TEST_F(TestObSSTableSchema, test_add_max_columns) {
  ...
  for (int i = 0; i < DEFAULT_COLUMN_DEF_SIZE; ++i) {
    column_def.column_group_id_ = 2;
    column_def.column_name_id_ = i + 2;
    if (common::OB_MAX_COLUMN_NUMBER > i) {
      column_def.table_id_ = 1000;
    } else {
...
TEST_F(TestObSSTableSchema, test_get_group_schema) {
  ...
  for (int table_id = 1000; table_id < 1010; ++table_id) {
    ...
    for (int group_id = 1 ; group_id <= (table_id - 1000) % 3 + 1; ++group_id) {
      column_def.column_group_id_ = group_id;
...
TEST_F(TestObSSTableSchema, test_get_table_column_groups_id) {
  ...
  uint64_t find[common::OB_MAX_COLUMN_GROUP_NUMBER];
  ...
  for (int i = 1000; i < 1010; ++i) {
    for (int ci = 0; ci < (i - 1000) % 3 + 1; ++ci) {
      size = common::OB_MAX_COLUMN_GROUP_NUMBER;
...
TEST_F(TestObSSTableSchema, test_compatible) {
  ...
  int64_t buf_size = common::OB_MAX_COLUMN_NUMBER * sizeof(ObSSTableSchemaColumnDefV1)
  ...
  for (int i = 0; i < common::OB_MAX_COLUMN_NUMBER; ++i) {
    ...
    columnV2_find = schemaV2.get_column_def(
                      schemaV2.find_offset_first_column_group_schema(1000, i + 2, group_id));
  ...
  uint64_t group_ids[common::OB_MAX_COLUMN_GROUP_NUMBER];
  int64_t size_group_id = common::OB_MAX_COLUMN_GROUP_NUMBER;
  int ret = OB_ERROR;
  for (int i = 0; i < common::OB_MAX_COLUMN_NUMBER; ++i) {
    //Get column group id
    size_group_id = common::OB_MAX_COLUMN_NUMBER;
...
TEST_F(TestObSSTableSchema, test_serialize_and_deserialize) {
  ...
  for (int i = 0; i < DEFAULT_COLUMN_DEF_SIZE; ++i) {
    column_def.column_group_id_ = (i % 128) / 8;
  ...
  for (uint64_t table_id = 1000; table_id < 1100; ++table_id) {
    for (uint64_t group_id = 0; group_id < 16; ++group_id) {
      ...
      EXPECT_EQ(def, schema.get_column_def(group_id * 8 + (table_id - 1000) * 128));
    }
  }
  ...
  for (uint64_t table_id = 1000; table_id < 1100; ++table_id) {
    for (uint64_t group_id = 0; group_id < 16; ++group_id) {
      ...
      EXPECT_EQ(group_size, 8);
      EXPECT_EQ(def, deserialized_schema.get_column_def(group_id * 8 + (table_id - 1000) * 128));
    }
  }
