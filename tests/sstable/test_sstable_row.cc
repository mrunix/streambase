 ...
 *
 * test_sstable_row.cc for test sstable row structure
 *
 * Authors:
...
TEST_F(TestObSSTableRow, test_add_objs) {
  ...
  EXPECT_EQ(5, row.get_column_group_id());
}
...
TEST_F(TestObSSTableRow, test_add_max_objs) {
  ...
  EXPECT_EQ(tmp_key.key_len(), row_key.length());
  EXPECT_TRUE(key == row_key);
  EXPECT_EQ(1000, row.get_table_id());
  EXPECT_EQ(5, row.get_column_group_id());
...
TEST_F(TestObSSTableRow, test_add_too_many_objs) {
  ...
  for (int i = 0; i < common::OB_MAX_COLUMN_NUMBER + 5; ++i) {
    tmp_obj.set_double(i);
  ...
  obj = row.get_obj(common::OB_MAX_COLUMN_NUMBER + 10);
  EXPECT_TRUE(NULL == obj);
  obj = row.get_obj(common::OB_MAX_COLUMN_NUMBER + 100);
  EXPECT_TRUE(NULL == obj);
  obj = row.get_obj(-1);
  EXPECT_TRUE(NULL == obj);
}
...
TEST_F(TestObSSTableRow, test_set_obj_count) {
  ...
  ret = row.set_obj_count(common::OB_MAX_COLUMN_NUMBER);
  EXPECT_TRUE(ret == OB_SUCCESS);
  EXPECT_EQ(common::OB_MAX_COLUMN_NUMBER, row.get_obj_count());
  ...
  EXPECT_TRUE(ret == OB_ERROR);
  EXPECT_EQ(0, row.get_obj_count());
  ret = row.set_obj_count(common::OB_MAX_COLUMN_NUMBER + 1);
  EXPECT_TRUE(ret == OB_ERROR);
  EXPECT_EQ(0, row.get_obj_count());
  ret = row.set_obj_count(common::OB_MAX_COLUMN_NUMBER + 10);
  EXPECT_TRUE(ret == OB_ERROR);
  EXPECT_EQ(0, row.get_obj_count());
}
...
TEST_F(TestObSSTableRow, test_check_schema_one_obj) {
  ...
  EXPECT_EQ(1000, row.get_table_id());
  EXPECT_EQ(2, row.get_column_group_id());
...
TEST_F(TestObSSTableRow, test_check_schema_with_multi_table) {
  ...
  EXPECT_EQ(common::OB_MAX_COLUMN_NUMBER * 7, schema.get_column_count());
...
TEST_F(TestObSSTableRow, test_serialize_and_deserialize) {
  ...
  pos = 0;
  deserialized_row.set_obj_count(common::OB_MAX_COLUMN_NUMBER);
  EXPECT_EQ(common::OB_MAX_COLUMN_NUMBER, deserialized_row.get_obj_count());
