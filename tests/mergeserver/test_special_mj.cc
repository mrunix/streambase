...
TEST(GroupBy, all_in_one_group) {
  ...
  EXPECT_EQ(scan_param.add_aggregate_column(column_name, as_column_name, COUNT), OB_SUCCESS);


  str.assign(const_cast<char*>("sum"), strlen("sum"));
  EXPECT_EQ(buffer.write_string(str, &as_column_name), OB_SUCCESS);
  EXPECT_EQ(scan_param.add_aggregate_column(column_name, as_column_name, SUM), OB_SUCCESS);

  str.assign(const_cast<char*>("max"), strlen("max"));
  EXPECT_EQ(buffer.write_string(str, &as_column_name), OB_SUCCESS);
  EXPECT_EQ(scan_param.add_aggregate_column(column_name, as_column_name, MAX), OB_SUCCESS);

  str.assign(const_cast<char*>("min"), strlen("min"));
  EXPECT_EQ(buffer.write_string(str, &as_column_name), OB_SUCCESS);
  EXPECT_EQ(scan_param.add_aggregate_column(column_name, as_column_name, MIN), OB_SUCCESS);
...
TEST(select_all, select_all_and_filter) {
  ...
  ObObj cond_obj;
  cond_obj.set_int(4);
  EXPECT_EQ(scan_param.add_cond(column_name, GT, cond_obj), OB_SUCCESS);
  ObScanner result;
  EXPECT_EQ(g_ms_client.ups_scan(scan_param, result, TIMEOUT), OB_SUCCESS);
}
