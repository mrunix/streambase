...
int TaskFactory::init_scan_param(const uint64_t table_id, uint64_t& max_len, ObScanParam& param) {
  // init version range
...
int TaskFactory::get_table_tablet(const char* table_name, const uint64_t table_id, uint64_t& count) {
  ...
  if (NULL == table_name) {
    ...
    ret = init_scan_param(table_id, max_len, scan_param);
    if (ret != OB_SUCCESS) {
              ...
              if (OB_SUCCESS == ret) {
                if (0 == port) {
                  ...
                  if (OB_SUCCESS != (ret = list.add(addr))) {
                    ...
                    TBSYS_LOG(DEBUG, "add addr succ:server[%s]", server_addr);
                  }
                }
                ip = port = 0;
              }
